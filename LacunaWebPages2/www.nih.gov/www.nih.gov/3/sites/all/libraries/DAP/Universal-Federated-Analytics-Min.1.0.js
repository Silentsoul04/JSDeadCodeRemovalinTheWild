...
function _initElements() {
                                            ...
                                            if (_thisElement.split("=")[0] == "autotracker") {
                                                ...
                                                if (_thisElement.split("=")[0] == "optout") {
                                                    ...
                                                    if (_thisElement.split("=")[0] == "fedagencydim") {
                                                        ...
                                                        if (_thisElement.split("=")[0] == "fedsubagencydim") {
                                                            ...
                                                            if (_thisElement.split("=")[0] == "fedversiondim") {
                                                                ...
                                                                if (_thisElement.split("=")[0] == "palagencydim") {
                                                                    ...
                                                                    if (_thisElement.split("=")[0] == "palsubagencydim") {
                                                                        ...
                                                                        if (_thisElement.split("=")[0] == "palversiondim") {
    ...
    if (oCONFIG.SUBDOMAIN_BASED) {
        var c = "";
        try {
            c = document.location.hostname.match(/(([^.\/]+\.[^.\/]{2,3}\.[^.\/]{2})|(([^.\/]+\.)[^.\/]{2,4}([^.\/]+\.)[^.\/]{2,4})|(([^.\/]+\.)([^.\/]+\.)[^.\/]{2,4}([^.\/]+\.)[^.\/]{2,4}))(\/.*)?$/)[1]
        } catch (f) {
            c = "www." + document.location.hostname.match(/(([^.\/]+\.[^.\/]{2,3}\.[^.\/]{2})|(([^.\/]+\.)[^.\/]{2,4}))(\/.*)?$/)[1]
        }
        oCONFIG.HOST_DOMAIN_OR = c
    }
...
function _sendCustomDimension(d, a) {
    if (d != "" && a != "") {
        d = d.split(",");
        for (var b = 0; b < d.length; b++) {
            if (d[b].indexOf("dimension") == 1) {
                d[b] = "dimension" + d[b]
            }
        }
        for (var c = 0; c < oCONFIG.GWT_UAID.length; c++) {
            if (c == 0) {
                if (d[0] != "dimension0") {
                    ga("GSA_ENOR0.set", d[0], a)
                }
            } else {
                if (d[1] != undefined && d[1] != "dimension0") {
                    ga(oCONFIG.PUA_NAME + c + ".set", d[1], a)
                }
            }
        }
    }
...
function _sendCustomMetrics(d, a) {
    if (d != "" && a != "") {
        d = d.split(",");
        for (var b = 0; b < d.length; b++) {
            if (d[b].indexOf("metric") == 1) {
                d[b] = "metric" + d[b]
            }
        }
        for (var c = 0; c < oCONFIG.GWT_UAID.length; c++) {
...
function _sendEvent(d, a, c, b) {
    if (d != "" && a != "") {
        ...
        for (var e = 0; e < oCONFIG.GWT_UAID.length; e++) {
            if (e == 0) {
                ga("GSA_ENOR0.send", "event", d, a, c, b)
            } else {
                ga(oCONFIG.PUA_NAME + e + ".send", "event", d, a, c, b)
            }
        }
    }
}

function _sendPageview(b, c) {
    if (b != "") {
        for (var a = 0; a < oCONFIG.GWT_UAID.length; a++) {
            if (a == 0) {
                if (c != "") {
                    ga("GSA_ENOR0.set", "title", c)
                } else {
                    ga("GSA_ENOR0.set", "title", document.title)
                }
                ga("GSA_ENOR0.send", "pageview", b)
            } else {
                if (c != "") {
                    ga(oCONFIG.PUA_NAME + a + ".set", "title", c)
                } else {
                    ga(oCONFIG.PUA_NAME + a + ".set", "title", document.title)
                }
                ga(oCONFIG.PUA_NAME + a + ".send", "pageview", b)
            }
        }
    }
}

function gas(g, c, d, f, e, b, a) {
    if (g == undefined) {
        g = ""
    }
    if (c == undefined) {
    ...
    if (c == "event") {
        ...
        _sendEvent(d, f, e, parseInt(b))
    } else {
        if (c == "pageview") {
            _sendPageview(d, f)
        } else {
            if (c.indexOf("dimension") > -1) {
                _sendCustomDimension(c, d)
            } else {
                if (c.indexOf("metric") > -1) {
                    _sendCustomMetrics(c, d)
                }
            }
        }
    }
...
function _URIHandler(a) {
    ...
    if (b.test(a)) {
        a = a.replace(b, "$1query$3")
    }
    return a
}(function(f, c, h, e, b, d, j) {
    f.GoogleAnalyticsObject = b;
    f[b] = f[b] || function() {
        (f[b].q = f[b].q || []).push(arguments)
    }, f[b].l = 1 * new Date();
    d = c.createElement(h), j = c.getElementsByTagName(h)[0];
    d.async = 1;
    d.src = e;
...
function _initAutoTracker() {
    ...
    for (var i = 0; i < h.length; i++) {
        try {
            var e = h[i].getAttribute("href");
            var b = h[i];
            var a = b.hostname;
            var g = b.href.split(a)[1].split(/[#?&?]/)[0];
            var f = b.protocol;
            if (!a.match(/(.*)\.(.*)\.(.*)/g)) {
                a = "www." + a
            }
            var j = f + "//" + a + g;
            if (a != "" && g != "" && a != "www.") {
                if (a.toLowerCase().indexOf(l.WEBSITE_HOSTNAME) > -1) {
                    _isDownload(g, l.DOWNLOADS_EXTs) == true ? _addEventListener(b, l.DEBUGGING_MODE, l.DOWNLOADS_CATEGORY, _getDownloadExt(g, l.DOWNLOADS_EXTs), j, 0) : false
                } else {
                    _isDownload(g, l.DOWNLOADS_EXTs) && l.TRACK_OUTBOUND_DOWNLOADS == true ? _addEventListener(b, l.DEBUGGING_MODE, l.OUTBOUND_DOWNLOAD_CATEGORY, _getDownloadExt(g, l.DOWNLOADS_EXTs), j, 0) : _addEventListener(b, l.DEBUGGING_MODE, l.OUTBOUND_LINK_CATEGORY, a, g, 0)
                }
            } else {
                if (_isEmailAddr(e)) {
                    var d = _getEmailAddr(e);
                    var c = _getEmailAddrHost(d);
                    if (l.WEBSITE_HOSTNAME.replace("www.", "") == c) {
                        _addEventListener(b, l.DEBUGGING_MODE, l.EMAIL_CATEGORY, d, "", 0)
                    } else {
                        if (c.indexOf(l.WEBSITE_HOSTNAME) > -1) {
                            _addEventListener(b, l.DEBUGGING_MODE, l.EMAIL_CATEGORY, d, "", 0)
                        } else {
                            if (l.TRACK_OUTBOUND_EMAILS) {
                                _addEventListener(b, l.DEBUGGING_MODE, l.OUTBOUND_EMAIL_CATEGORY, d, "", 0)
                            }
                        }
                    }
                } else {
                    if (_isTelNum(e)) {
                        _addEventListener(b, l.DEBUGGING_MODE, l.TELEPHONE_CATEGORY, _getTelNum(e), "", 0)
                    }
                }
            }
        } catch (k) {}
...
function _isDownload(b, a) {
    if (b.toLowerCase().match(new RegExp("^(.*)(" + a + ")(.*)$")) != null) {
        return true
    } else {
        return false
    }
...
function _getDownloadExt(b, a) {
    return b.toLowerCase().match(new RegExp(".(" + a + ")"))[0].replace(".", "")
}

function _isEmailAddr(a) {
    if (a.toLowerCase().match(/^mailto\:[_a-zA-Z0-9-]+(\.[_a-zA-Z0-9-]+)*@[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*(\.[a-zA-Z]{2,4})/g)) {
        return true
    } else {
        return false
    }
...
function _getEmailAddr(a) {
    return a.toLowerCase().match(/[_a-zA-Z0-9-]+(\.[_a-zA-Z0-9-]+)*@[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*(\.[a-zA-Z]{2,4})/g).toString()
}

function _getEmailAddrHost(a) {
    return a.toString().split("@")[1]
}

function _isTelNum(a) {
    if (a.toLowerCase().match(/^tel\:(.*)([0-9]{3})\)?[-. ]?([0-9]{3})[-. ]?([0-9]{4})$/g)) {
        return true
    } else {
        return false
    }
...
function _getTelNum(b) {
    var a = b.toLowerCase().match(/^tel\:(.*)([0-9]{3})\)?[-. ]?([0-9]{3})[-. ]?([0-9]{4})$/g).toString();
    return a.toString().replace("tel:", "")
}

function _unionParams(a) {
    if (a == "true" || a == "on" || a == "yes" || a == "1") {
        return "true"
    } else {
        if (a == "false" || a == "off" || a == "no" || a == "0") {
            return "false"
        } else {
            return a
        }
...
function _addEventListener(a, b, d, c, f, e) {
    if (b == true) {
        var g = a.getAttribute("onmousedown");
        a.setAttribute("onmousedown", "_sendEvent('" + d + "','" + c + "','" + f + "'," + e + ");" + (g != null ? g : ""))
    } else {
        if (b == false) {
...
function _initYouTubeTracker() {
    ...
    for (var b = 0; b < a.length; b++) {
        _thisVideoObj = a[b];
        var e = _thisVideoObj.src;
        if (IsYouTube(e)) {
            _thisVideoObj.src = YTUrlHandler(e);
            var d = youtube_parser(e);
...
function onPlayerStateChange(c) {
    var d = c.target.getVideoUrl();
    var b = youtube_parser(d);
    if (c.data == YT.PlayerState.PLAYING) {
        _thisDuration = ((c.target.getCurrentTime() / c.target.getDuration()) * 100).toFixed();
        if (_thisDuration == 0) {
            _f33 = false;
            _f66 = false;
            _f90 = false
        }
        _sendEvent("YouTube Video", "play", d, 0)
    } else {
        if (c.data == YT.PlayerState.ENDED) {
            _sendEvent("YouTube Video", "finish", d, 0)
        } else {
            if (c.data == YT.PlayerState.PAUSED) {
                _sendEvent("YouTube Video", "pause", d, 0);
                var a = ((c.target.getCurrentTime() / c.target.getDuration()) * 100).toFixed();
                if (a < 100) {
                    var e = ((c.target.getCurrentTime() / c.target.getDuration()) * 100).toFixed();
                    if (e > 0 && e <= 33 && _f33 == false) {
                        _sendEvent("YouTube Video", "33%", d, 0)
                    } else {
                        if (e > 0 && e <= 66 && _f66 == false) {
                            _sendEvent("YouTube Video", "66%", d, 0)
                        } else {
                            if (e > 0 && e <= 90 && _f90 == false) {
                                _sendEvent("YouTube Video", "90%", d, 0)
                            }
                        }
                    }
                }
...
function _initIdAssigner() {
    ...
    for (var a = 0; a < c.length; a++) {
        var b = c[a].getAttribute("id");
        if (b == null || b == "") {
            c[a].setAttribute("id", "anch_" + a)
        }
    }
}

function CookieTimeoutCalc_Months(b) {
    var a = 60 * 60 * 24 * 30.416667;
    return a * b
}
