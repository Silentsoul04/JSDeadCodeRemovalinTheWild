...
/*[/img/js/dateSelect.js]*/
function DateSelector(startYear,endYear,zero){
	if(startYear == undefined) startYear = 2004;
	this.sYear =  startYear;
	if(endYear == undefined) endYear = 2015;
	this.eYear =  endYear;	
...
DateSelector.prototype = {
	inityear: function(jqueryObj, defaultValue){			
		//jqueryObj.append("<option value=''>--不限--</option>");
		defaultValue = (defaultValue == undefined) ? "0" : defaultValue;
		for (var i = this.eYear; i >= this.sYear; i--){		
	...
	initmonth: function(jqueryObj, defaultValue){			
		...
		for (var i = 1; i <= 12; i++){		
			if(this.zeroPre)
				...
				jqueryObj.append("<option value='" + ((i<10)? ("0" + i) : i)  + ((i != defaultValue)?"'>":"' selected>") +  i  + "</option>");
		}		
	},
...
/*[/img/js/mods_direction.js]*/
$(function(){
    window._video_play_status = 0;
    var conhtml = '',
        timeout = undefined,
        interval = 5000,
        $con = $('#bb-home-spotlight_con'),
	    direction_url = "/homepage/call.php";	

    function replace(){
        clearTimer();

        $con.find("ul.tweetList li:animated").stop(true, true);
        var html = $con.html();

		$con.find('.homeContBody').fadeOut(100, function(){
            $con.html(conhtml);
            conhtml =  html;
            $con.find('.homeContBody').hide().fadeIn(400);
            mods_direction_init($('#cur_direction_id').text());
            window._video_play_status = 0;
            setTimer();
        });
    }
    ...
    function setTimer(){
        timeout = setTimeout(function(){
            if (window._video_play_status) {return false;}
            var a = $con.find('.spotlightJump>li[class!=active]>a');
            a.trigger('click');
        }, interval);
    }

	//bind prev next pointer envent.
	$(document.body).delegate('#top_prev_direction_pointer, #top_next_direction_pointer',
            'click', function(){

		var direction_id = $(this).attr('direction_id');
		if(direction_id == undefined){
			return false;
		}

        var from = ($('#btn_shrink_content').length) ? '' : 'list';
		var url = direction_url+'?action=get_direction_html&id=' + direction_id + '&from=' + from;
		$.ajax({
			type: "GET",
			url: url,
			success: function(htmStr){
                var $wrap = $('#bb-home-spotlight_con');
				htmStr = htmStr.replace(/^<div id="bb-home-spotlight_con">|<\/div>$/g, '');
				$wrap.html(htmStr);
			}
		});
	});

	if($con.find('.spotlightJump>li[class=active]>a').length){
        ...
        $con.delegate('li[class!=active]>a', 'click', function(){
            var $this = $(this);
            if(conhtml){
                replace();
            }
                    ...
                    url: $this.attr('href'),
 	   				success: function(htmStr){
 	   					conhtml = htmStr.replace(/<div id="bb-home-spotlight_con">|<\/div>$/g, '');
                        replace();
 	   				}
 	   			});
            }
...
function mods_direction_init(curDirectionId){
	var shrink_time = 24*60*60*1000;//milliseconds
	var direction_url = "/homepage/call.php";	

	(function(){
		var cookie_name = 'is_vote_' + window.loginUid + '|' + $('#direction_vote_id').val();
		function getVoteResult(vote_selected_id_list){
			vote_selected_id_list = vote_selected_id_list == undefined ? '' : vote_selected_id_list;
			$.ajax({type: "POST",
					url: direction_url+'?action=vote',
					data: 'vote_id='+$('#direction_vote_id').val()+'&direction_vote_option_id_list=' + $('#direction_vote_option_id_list').val() + '&direction_vote_option_text_list=' + $('#direction_vote_option_text_list').val() + vote_selected_id_list,
					success: function(dt){
						eval('var dt='+dt);
						var vote_info_list_html = '';
						var text_len = 8;
						$(dt.data).each(function(idx, val){
							vote_info_list_html += '<dd><span style="display: inline-block;width: 115px;margin-right: 5px;">' + val.vote_caption.substr(0,text_len) + (val.vote_caption.length > text_len ? '...' : '') + '</span><span class="percentBar"><img src="' + $('#direction_vote_info_ratio_img').val() + '" class="percentBar" style="width:' + (val.view_length == undefined ? 0 : val.view_length) + 'px;" /></span><span class="percentage" style="color:#C30;">' + val.vote_ratio + '</span></dd>';
							});
						$('#vote_info_list').html(vote_info_list_html);
						$('#vote_info_list').css('visibility', 'visible');
						$('#btn_vote_submit').css('visibility', 'hidden');
		...
		if(window.login === true){
			if(getCookie(cookie_name) != ""){
				...
				var voteSelectedIdsStr = '';
				var matchArr = document.location.href.match(/(vote_selected_id\[\]=\d+)/ig);
				if(matchArr){
					var matchCount = matchArr.length;
					for(var i=0;i<matchCount;i++){
						voteSelectedIdsStr += '&' + matchArr[ i ];
					}
					if(voteSelectedIdsStr != ""){
						getVoteResult(voteSelectedIdsStr);
						return true;
					}
				}
			}
		}
		//bind btn vote submit
		$('#vote_info_list').css('visibility', 'visible');
		$('#btn_vote_submit').css('visibility', 'visible');
		$('#btn_vote_submit').bind('click', function(){
				var vote_selected_id_list = '';
				$('#vote_info_list input').each(function(){
					if($(this).attr('checked')){
						vote_selected_id_list += '&' + $(this).attr('name') + '=' + $(this).val();
					}
				});
				if(vote_selected_id_list == ''){
					alert('请选择投票选项！');
					return false;
				}
				if(window.login !== true){
					var cur_url = document.location.href;
					cur_url = cur_url.replace(/#issue-feature-\d+/g, '');
					if(cur_url.indexOf('?') === -1){
						vote_selected_id_list = vote_selected_id_list.substr(1);
						vote_selected_id_list = '?' + vote_selected_id_list;
					}
					window.location.href = 'http://www.babytree.com/reg/login.php?type=sybox&url='+encodeURIComponent(cur_url + vote_selected_id_list);
					return false;
				}	
				getVoteResult(vote_selected_id_list);	
		});
	}());

	//create twitter scroll
    var twitter_num = $('#bb-home-spotlight .tweetList').children().length;
    if (twitter_num > 2) {
	    var twitter_scroll = new TwitterScroll('bb-home-spotlight');
    } else {
	    var twitter_scroll = new Array();
    }

	//set twitter comment and forword display or hidden.
    $('div#bb-home-spotlight ul.tweetList li').hover(function(){
	    $(this).find('div.tweetFunc').show();
	},function(){
	    $(this).find('div.tweetFunc').hide();
	});
 
	(function(){ 
	 
	 //bind shrink and open button click event.
	 $('#btn_shrink_content, #btn_open_content').click(function(){
		 var id = $(this).attr('id');
		 var idClassMapping = {
		 "btn_shrink_content": {cls: "homeContBlk", callBack: function(){
			$('#bb-home-spotlight').attr('class', this.cls);
			setCookie('direction_switch_info', curDirectionId + '|' + (cur_date.getTime() + (shrink_time+1))/*id|milliseconeds*/, 30);
			twitter_scroll.pause = true;
			if(window.direction_scroll_img != undefined){
			window.direction_scroll_img.pause = true;
			}
			}},
		 "btn_open_content": {cls: "homeContBlk spotlightExtended", callBack: function(){
			$('#bb-home-spotlight').attr('class', this.cls);
			setCookie('direction_switch_info', curDirectionId + '|' + (cur_date.getTime() - shrink_time)/*id|milliseconeds*/, 30);
			if(window.direction_scroll_img != undefined){
			window.direction_scroll_img.pause = false;
			}
			twitter_scroll.pause = false;
			}}
		 };
		 idClassMapping[ id ].callBack(); 
	 });
	 
	 //set cookie to display or hide the direction content;	
	 var cur_date = new Date();
     	 var direcion_switch_info_arr = getCookie('direction_switch_info').split('|');
	 if(direcion_switch_info_arr[0] != null && direcion_switch_info_arr[1] != null && direcion_switch_info_arr[0] >= curDirectionId){
	 if((direcion_switch_info_arr[0] == curDirectionId) 
		 && (parseInt(direcion_switch_info_arr[1]) > cur_date.getTime())){
         $('#btn_shrink_content').click();
	 }
	 }else{
         $('#btn_open_content').click();
	 } 
	})();
...
//风向标滚动
function getRollList(type) {
    var start = parseInt($('#roll_ul').attr('start'));
    if (type == 'prev') {
        start--;
    } else {
        start++;
    ...
    if (start < 1 ) {
        ...
        $.get('direction_ajax.php?action=get_direction&pg='+start,function(dt){
            var json = $.parseJSON(dt);
            if (json != 'null') {
                var roll_li = '';
                var roll_num = json.rows;
                if (roll_num > 0) {
                    $.each(json.data, function(i,v){
                        if (i == 0) {
                           li_class = 'first';
                        } else if ((i+1) == roll_num) {
                            li_class = 'end';
                        } else {
                            li_class = '';
                        }
                        roll_li += '<li class="'+li_class+'"><a href="/homepage/call.php?action=get_direction_html&from=list&id='+v.id+'" class="fancy_box">';
                        roll_li += '<b><em></em>'+v.period_name+'</b>'+v.title+'</a></li>';
                    });
                    $('#roll_ul').html(roll_li);
                    $('#roll_ul').attr('start',start);
                    $(".fancy_box").fancybox({autoScale : false, scrolling : 'no', titleShow : false});
                }
            }
        });
    }
...
//显示切换微博
function showWeibo(li) {
    var id = li.attr('id');
    $.each(['learn','mika','ask'],function(i,value){
        if (id == value) {
            $('#'+id+'_weibo').show();
            $('#'+id).addClass('current');
        } else {
...
function submitTopic() {
    ...
    if (content == '') {
        ...
        $.post('direction_ajax.php?action=submit_topic',{topic:content},function(data){
            if (data == 'login') {
                reg_show_popup('login',submitTopic,'login','');
...
;
/*[/img/js/homepage2011.js]*/
	$(function(){
        //tweet quee - mods_nest_five
		$(".jCarouselLiteTweet").jCarouselLite(
			{
				auto: 2000,
				vertical: true,
				circular: true,
		...
		$("#jCarouselLiteTweet li").hover(
			function(){
				$(this).children("div.tweetBody").children(".tweetFunc").show();
			}
		,
			function(){
				$(this).children("div.tweetBody").children(".tweetFunc").hide();
			}
		);
		//$("#jCarouselLiteTweet div.tweetFunc").show()
		//$("#jCarouselLiteTweet div.tweetFunc").hide()
        //
        //mods_rec_tpl
        $(".jCarouselLite").jCarouselLite(
			{
				btnNext:".entranceScroll .next_btn_spacer",
				btnPrev:".entranceScroll .prev_btn_spacer",
				circular: false,
				visible: 6 
			}
		);
...
//mods_know_tpl
function switchKnowTab(tab)
{
	tmp = 0;
	while(tmp++ < 5){
		document.getElementById("fc_tb1_"+tmp).style.display = "none"; 
		document.getElementById("fc_tb2_"+tmp).style.display = "none";
		document.getElementById("fc_tb3_"+tmp).style.display = "none";
		document.getElementById("fc_tbh_"+tmp).className = "";
	}

	document.getElementById("fc_tb1_"+tab).style.display = ""; 
	document.getElementById("fc_tb2_"+tab).style.display = "";
	document.getElementById("fc_tb3_"+tab).style.display = "";
	document.getElementById("fc_tbh_"+tab).className = "current";
}


$(function(){
	$("#navAddPopup span.dismiss").click(function(){ $("#navAddPopup").hide();$("#addMyLinkBt").removeClass("navAddOn");});
             
	$("#navAddPopup input[name=inputLink]").click(function(){
		if($(this).val() == "请填入一个网址(URL)")
			$(this).val("");
	});
	$("#navAddPopup input[name=inputName]").click(function(){
		if($(this).val() == "起个名字吧")
			$(this).val("");
	});
	$("#navAddPopup button[name=inputSubmit]").click(function(){
		ULName = $("#navAddPopup input[name=inputName]").val();
		if(ULName== "" || ULName == "起个名字吧"){
			alert("起个名字吧");
			return false;
		}
		if(ULName.replace(/[^\x00-\xff]/g,"**").length > 12){
			alert("收藏的链接名字长度不能超过6个汉字或12个英文字符");
			return false;
		}

		ULLink = $("#navAddPopup input[name=inputLink]").val();
		if(!IsURL(ULLink)){
			alert("请填入一个规范的网址(URL)");
			return false;
		}
		if(ULLink.indexOf("://") < 0){
            ULLink = "http://" + ULLink;
		...
		$.ajax({
		     url:"/homepage/call.php",
		     data:"action=add&name="+encodeURIComponent(ULName)+"&link="+encodeURIComponent(ULLink),
		     beforeSend:function(){ },
		     error: function(xhr, textStatus, errorThrown) { alert("请求失败，请重试……"); },
		     success:function(msg){
                    $("#navAddPopup").hide();$("#addMyLinkBt").removeClass("navAddOn");

					if(msg == "over"){
						alert("收藏失败，最多只能收藏"+userLinkCountMax+"条网址。");
					}else{
						$("#navAddPopup input[name=inputLink]").val("请填入一个网址(URL)");
						$("#navAddPopup input[name=inputName]").val("起个名字吧");
						tmp = '<dd id="rrd_'+msg+'"><a target="_blank" href="'+ULLink+'">'+ULName+'</a><span style="display:none" class="dismiss" onclick="removeUserLink('+msg+')" title="删除"></span></dd>';
						$("#navEditPanel").append($(tmp));
						$("#navEditPanel dd:last-child").hover(function(){$(this).children(".dismiss").show()},function(){$(this).children(".dismiss").hide()});
						userLinkCount ++;
                        checkNavEditEnable();
					}
		         }
	...
	$("#addMyLinkBt").click(function(){
       $("#navAddPopup .count").html(userLinkCountMax - userLinkCount);
       if($(this).hasClass("navAddOn")){
           $(this).removeClass("navAddOn");
	       $("#navAddPopup").hide();
       }else{
           $(this).addClass("navAddOn");
	       $("#navAddPopup").show();
           $("#editMyLinkBt").removeClass("navEditOn");$("#navEditPanel").removeClass("homeNavCustomOn");
       }
	});
    dismiss_show_in_navEidtPanel = false;
	$("#editMyLinkBt").click(function(){ 
		if($("#navEditPanel").hasClass("homeNavCustomOn")) { dismiss_show_in_navEidtPanel= false;$(this).removeClass("navEditOn");$("#navEditPanel").removeClass("homeNavCustomOn");}else{dismiss_show_in_navEidtPanel = true ;$(this).addClass("navEditOn");$("#navEditPanel").addClass("homeNavCustomOn");
       $("#navAddPopup").hide();$("#addMyLinkBt").removeClass("navAddOn");
        }});

	$("#navEditPanel dd span.dismiss").hide();
	$("#navEditPanel dd").hover(function(){ if(dismiss_show_in_navEidtPanel) $(this).children(".dismiss").show()},function(){$(this).children(".dismiss").hide()});

    checkNavEditEnable();
});

function checkNavEditEnable(){
    if(userLinkCount >= userLinkCountMax){
        $("#addMyLinkBt").hide();
    }else{
        $("#addMyLinkBt").show();
    }
    if(userLinkCount > 0){
        ...
        $("#editMyLinkBt").hide();
        $("#navEditPanel").removeClass("homeNavCustomOn");
    }
}
...
function highLightAgeLabel(year,month)
{
    var o = document.getElementById("age_e_"+year+"_"+month);
    if(o == undefined || o == null)
        o = document.getElementById("age_e_"+year+"_"+month);
    if(o == undefined || o == null)
        return ;
    o.className = "current";
}
...
function showLinkTips(kname)
{
    var tmp = parseInt(getCookie(kname));
    if(tmp) return ;
    $("#popupTip").show();
    var height_tp = $("#addMyLinkBt").height();
    var top_tp = $("#addMyLinkBt").offset().top;
    var left_tp = $("#addMyLinkBt").offset().left;
    $("#popupTip").css("left",left_tp+40);
    $("#popupTip").css("top",top_tp-8);
    $("#popupTip .popupClose").click(function(){closeLinkTips(kname);});
}

function closeLinkTips(kname)
{
    $("#popupTip").hide();
    date = new Date();
    var ms = 86400*365*1000;
    date.setTime(date.getTime() + ms);
    document.cookie = kname+"=1; expires=" + date.toGMTString() + "; domain=babytree.com";
}
/*[/img/js/twitter_scroll.js]*/
;
//twitter info scroll.
function TwitterScroll (conId){
	var that = this,
        panel = $('#' + conId + ' ul.tweetList');
	...
	function scroll() {
		...
		var lastTwitter = panel.find('li:last');
        lastTwitter.hide();
        panel.prepend(lastTwitter);
        lastTwitter.slideDown(600);
        //lastTwitter.fadeIn();
	}

	if(window.tscrollInterval){
		window.clearInterval(window.tscrollInterval);
	}

	window.tscrollInterval = window.setInterval(scroll, 4000);
...
;
/*[/static/bui/extension/header/globalHeader.js]*/
/**
