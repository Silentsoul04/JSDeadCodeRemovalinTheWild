...
if (YAHOO.ywa.I13N === undefined) {
    YAHOO.ywa.I13N = (function() {
        ...
        initializePixelConfigs = function() {
            numPixelConfigsRemaining = 0;
            if (window.dotq) {
                for (var i = 0; i < window.dotq.length; i++) {
                    if (window.dotq[i] !== undefined && window.dotq[i].properties !== undefined && window.dotq[i].properties.pixelId !== undefined && InternalConf.pixelConfigs[window.dotq[i].properties.pixelId] === undefined) {
                        numPixelConfigsRemaining++;
                        InternalConf.pixelConfigs[window.dotq[i].properties.pixelId] = {};
                    }
                }
            }
        };
        ...
        initializeCMP = function() {
            ...
            window.__cmp("getConsentData", null, function(result) {
                if (result.consentData) {
                    InternalConf.gdpr_consent = result.consentData;
                }
                if (result.gdprApplies !== undefined) {
                    InternalConf.gdpr = (result.gdprApplies == true ? 1 : 0);
                }
                if (result.isOathFirstParty) {
                    InternalConf.isOathFirstParty = (result.isOathFirstParty == true ? 1 : 0);
                }
                consentReady = true;
                continueIfInitialized();
            });
        ...
        initialize = function() {
            ...
            initializeCMP();
            loadPixelConfigurations();
        };
        continueIfInitialized = function() {
            if (consentReady && numPixelConfigsRemaining === 0) {
                while (consentReadyCallbacks.length) {
                    try {
                        consentReadyCallbacks.shift()();
                    } catch (e) {}
                }
            }
        };
        ...
        generateBeaconDescriptors = function(beaconDescriptors) {
            var descriptors, convertStringPidToPidList, convertPidListToProjectList, convertProjectListToBeaconList, purgeInvalidDescriptors;
            ...
            addIdsToBeaconDescriptors(beaconDescriptors);
            return beaconDescriptors;
        };
        addIdToBeaconDescriptors = function(beaconDescriptors, idKey, idValue) {
            var idx;
            if (idValue === undefined || idValue === "") {
                return;
            }
            for (idx = 0; idx < beaconDescriptors.length; idx += 1) {
                var beaconParameters = beaconDescriptors[idx].properties;
                beaconParameters.qstrings = beaconParameters.qstrings || {};
                beaconParameters.qstrings[idKey] = idValue;
            }
        };
        addIdsToBeaconDescriptors = function(beaconDescriptors) {
            var clickId, vmuuid;
            clickId = getVmId("vmcid");
            if (clickId === undefined || clickId === "") {
                return;
            }
            vmuuid = getVmId("vmuuid");
            if (vmuuid === undefined || vmuuid === "") {
                vmuuid = generateVmuuid();
            }
            addIdToBeaconDescriptors(beaconDescriptors, "vmcid", clickId);
            addIdToBeaconDescriptors(beaconDescriptors, "vmuuid", vmuuid);
        };
        ...
        getParamValue = function(paramValueString, param, separator) {
            var valueRegExp = new RegExp(separator + param + "=([^" + separator + "]*)");
            var matchRes = (separator + paramValueString).match(valueRegExp);
            if (matchRes === null) {
                return undefined;
            }
            return matchRes[1];
        };
        onPixelConfigResponse = function() {
            if (this.readyState === this.DONE) {
                if (this.status === 200) {
                    if (this.responseText && this.responseText !== "{}") {
                        try {
                            var configObject = JSON.parse(this.responseText);
                            InternalConf.pixelConfigs[configObject.pixelId] = configObject;
                            if (overridePixelConfig === undefined || overridePixelConfig.use1stPartyCookies !== true) {
                                overridePixelConfig = configObject;
                            }
                        } catch (e) {}
                    }
                }
        ...
        loadPixelConfigurations = function() {
            for (var pixelId in InternalConf.pixelConfigs) {
                loadPixelConfig(pixelId);
            }
            waitForInitialization(storeClickId);
        };
        loadPixelConfig = function(pixelId) {
            if (pixelId === undefined) {
                return;
            }
            var http = new XMLHttpRequest();
            var url = "https://s.yimg.com/wi/config/" + pixelId + ".json";
            ...
            http.send();
            http.onreadystatechange = onPixelConfigResponse;
        };
        isUse1stPartyCookies = function() {
            if (overridePixelConfig === undefined) {
                return defaultPixelConfig.use1stPartyCookies;
            } else {
                return overridePixelConfig.use1stPartyCookies;
            }
        };
        storeId = function(idKey, idValue) {
            setCookie({
                name: idKey,
                value: idValue,
                expiryOffset: clickIdExpiryTime,
                domain: "",
                path: "/"
            });
            var idWithTimestamp = {
            ...
            localStorage.setItem(idKey, JSON.stringify(idWithTimestamp));
        };
        storeClickId = function() {
            if (isUse1stPartyCookies()) {
                var clickId = getParamValue(window.location.search.substring(1), "vmcid", "&");
                if (clickId === undefined || clickId === "") {
                    return;
                }
                storeId("vmcid", clickId);
                var vmuuid = getVmId("vmuuid");
                if (vmuuid === undefined || vmuuid === "") {
                    vmuuid = generateVmuuid();
                }
                storeId("vmuuid", vmuuid);
            }
        };
        getVmId = function(idKey) {
            var cookieString = (document.cookie || "").replace(/\s/g, "");
            var id = getParamValue(cookieString, idKey, ";");
            if (id === undefined || id === "") {
                var idObject = localStorage.getItem(idKey);
                if (idObject !== null) {
                    try {
                    ...
                    if (new Date().getTime() <= (idObject.timestamp + (clickIdExpiryTime * 1000))) {
                        id = idObject.id;
                    }
                }
            }
        ...
        Public = {
            modules: {},
            version: "1.1.0",
            fireBeacon: function(beaconConfigs) {
            ...
            getBeaconUrl: function(beaconDescriptor) {
                ...
                if (InternalConf.gdpr !== undefined) {
                    beaconDescriptor.properties.gdpr = InternalConf.gdpr;
                }
                if (InternalConf.gdpr_consent) {
                    beaconDescriptor.properties.gdpr_consent = InternalConf.gdpr_consent;
                }
                if (InternalConf.isOathFirstParty) {
                    beaconDescriptor.properties.isOathFirstParty = InternalConf.isOathFirstParty;
                }
                ...
                for (idx = 0, paramsLength = moduleParams.length; idx < paramsLength; idx += 1) {
                    ...
                    if (moduleParam.name !== undefined && moduleParam.value !== undefined) {
        ...
        Public = {
            getQueryParams: function(beaconParameters) {
                ...
                if (!beaconParameters.pixelId) {
                    throw {
                        message: "Mandatory property is missing: pixelId",
                        faultCode: -1
                    };
                }
                paramMap = {
                ...
                priorityMap = ["documentName", "pixelId", "documentGroup", "url", "referrer", "pageEncoding", "gdpr", "gdpr_consent", "isOathFirstParty"];
                params = [];
                for (idx = 0, pl = priorityMap.length; idx < pl; idx += 1) {
                    ...
                    if (beaconParameters.hasOwnProperty(propertyName) && paramMap.hasOwnProperty(propertyName) && beaconParameters[propertyName] !== "") {
                        params.push({
                            name: paramMap[propertyName],
                            value: beaconParameters[propertyName]
                        });
                    }
                }
                var isIframe = false;
