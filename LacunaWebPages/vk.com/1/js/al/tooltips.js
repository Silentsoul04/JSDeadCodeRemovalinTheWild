var tooltips = {
  show: function(el, options) {
    /**
     * When we show tooltip in audio popup we need to force fix friends block in Edge.
     * For more details look at MUSIC-6907
     */
    window.AudioPage && window.AudioPage.forceFix && window.AudioPage.forceFix();
    ...
    el.ttimer = setTimeout(function() {
      ...
      var st = (bodyNode.scrollTop || htmlNode.scrollTop || 0), ttsize = getSize(container),
  ...
  rePositionTT: function(tt) {
    ...
    setStyle(container, {left: newleft, top: newTop});
  },
  ...
  destroy: function(el) {
    ...
    if (el.tt && el.tt.el) {
      ...
      removeEvent(el, 'click', el.tthide);
      removeEvent(window, 'scroll', el.tthide);
      removeEvent(document, 'click', el.tthide);
  ...
  create: function(el, options) {
    ...
    if (!el.tthide) {
      ...
      if (!options.nohide) {
        addEvent(el, 'mouseout', el.tthide);
        addEvent(el, 'click', el.tthide);
        addEvent(window, 'scroll', el.tthide);

        if (!options.hasover) {
          addEvent(document, 'click', el.tthide);
        }
      }
    }
    ...
    <tr><td colspan="3" class="tt_bottom"><div class="bottom_sh"></div><div class="bottom_pointer'+no_shadow+'"></div></td></tr>\
      ...
      if (options.appendEl) {
        ...
        if (!appendEl) {
          do {
            appendEl = domClosestPositioned(appendEl || el, {noOverflow: true});
