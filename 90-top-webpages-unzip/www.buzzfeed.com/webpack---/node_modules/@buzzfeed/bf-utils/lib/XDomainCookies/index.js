...
const rejectOnTimeout = function(timeout) {
  return new Promise((_, reject) => {
    const error = { type: 'timeout', msg: `${timeout}ms timeout exceeded`, };
...
export default class XDomainCookies {
   ...
   */
  set({ name, value, days, secureOnly = true }) {
    // If we're in the same domain as the source of truth, use local cookies
    if (this.sourceOfTruthDomain === this.localDomain) {
      cookies.set({ name, value, days, localDomain: this.localDomain });
      return;
    }
    // Otherwise wait for iframe
    this.initIframe().then(() => {
      ...
      .catch(() => cookies.set({ name, value, days, localDomain: this.localDomain }));
  }
  ...
  cleanup() {
    ...
    if (this.iframe) {
      ...
      const event = new ErrorEvent({ message: 'XDomainCookies were cleaned up before ready' });
