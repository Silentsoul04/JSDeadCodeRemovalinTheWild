...
import { getUniqueEventName } from '../../services/util/standalone';
...
export default class Sticky {
  constructor(context) {
    ...
    this.config = {...configDefaults, ...context.getConfig('stickToTop')};
  ...
  manageSticky({force = false} = {}) {
    if (!this.canStick) {
      return;
    }

    const { inViewport } = this.getPosition();
  ...
  adjustFixedPosition = ({priority}) => {
    ...
    if (allowedTop !== this.fixAt) {
      ...
      this.manageSticky({force: true});
    }
  }

  checkEligibility = ({breakpoint} = {}) => {
    let canStick;
    if (this.config.breakpoints.indexOf('ALL') > -1) {
      canStick = true;
    } else {
