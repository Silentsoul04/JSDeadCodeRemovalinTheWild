...
import { Application, EventTarget } from 't3js';
...
const service = {
  ...
  getFixedRect(element, {priority = PRIORITY.normal, requestedTop = 'auto'} = {}) {
    priority = service.validatePriority(priority);
    const rect = getBoundingRect(element);
    let allowedTop;
    if (requestedTop === 'auto') {
      allowedTop = service.getAvailableTop(element, {priority, boundingRect: rect});
    } else {
   ...
   */
  add(element, {priority = PRIORITY.normal, requestedTop = 'auto'} = {}) {
    if (stickyEls.has(element)) {
      return service.update(element);
    }
    priority = service.validatePriority(priority);
    const rect = service.getFixedRect(element, { priority, requestedTop });
    stickyEls.set(element, {rect, priority, requestedTop});
  ...
  remove(element) {
    ...
    if (!element.className.includes('sticky--fixed sticky--show')) { stickyEls.delete(element); }
   ...
   */
  getAvailableTop(element, {priority = PRIORITY.normal, boundingRect} = {}) {
    priority = service.validatePriority(priority);
    const higher = [];
    stickyEls.forEach((settings, el) => {
    ...
    return Math.max(...stickyOverlapping.map(({bottom}) => bottom));
  },
