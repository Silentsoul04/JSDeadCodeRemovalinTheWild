...
import { buzzes } from '@buzzfeed/user-pathing/session';
...
const makeDataSource = ({ data_source_metadata = {} } = {}) =>
  encodeURIComponent(
    JSON.stringify({
      upstream: 'recsys_api',
 ...
 */
export function fetch(endpoint = 'web/bpager', { buzzId = bzfd.buzzId, edition = localization.getEdition(), imageCrop = 'big', pageSize = 5, filter = buzzes.all(), ...params } = {}) {
  return Promise.race([
    getUserId({ env: BZFD.Config.env }),
    ...
    const query = toQueryString({ edition, pageSize, buzzId, filter, ...params });
    const url = BZFD.Config.recsys_api_origin + '/' + endpoint + query;

    return ajax.get(url, { skipAuth: true }).then(mapResults({ imageCrop }));
  });
}
