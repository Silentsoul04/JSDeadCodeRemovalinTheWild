...
function ATagWithInternalLinkHandler(element, executable) {
  ...
  var trackingInfo = ["track/click/Nav-Link", { t: "click", n: "Nav-Link", l: parser.findLocation(element),
    data: {url: getValidInternalUrlString(urlObj), }, }, ];
...
function SelectTagChangedClickHandler(element, executable) {
  ...
  var trackingInfo = ["track/click/" + parsed.action, { t: "click", n: parsed.action, l: parsed.location,
    d: parsed.description, data: parsed.data, opt: parsed.options, }, ];
  if (executable && window[executable] instanceof Function) {
    window[executable].apply(this, trackingInfo);
  }
  return true;
...
function ATagDefaultHandler(element, executable, trackingMethod) {
  ...
  var trackingInfo = ["track/" + trackingMethod + "/" + parsed.action, { t: trackingMethod, n: parsed.action,
    l: parsed.location, d: parsed.description, data: parsed.data, opt: parsed.options, }, ];
  if (executable && window[executable] instanceof Function) {
    window[executable].apply(this, trackingInfo);
  }
  return true;
