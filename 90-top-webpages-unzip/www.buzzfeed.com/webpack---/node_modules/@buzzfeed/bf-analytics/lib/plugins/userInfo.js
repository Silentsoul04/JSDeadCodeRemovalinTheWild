...
import { defaultsDeep, isPlainObject, merge, cloneDeep, } from "lodash";
...
function UserInfoPlugin(core, settings){
  ...
  core._router.sub("general/user/{action}", function(dataObj, { action }){
    if (action !== "update") {
    ...
    if (!this.settings.isReady && this.settings.readyOnFirstUpdate) {
      getUserId({ env: core._generalSettings.env })
        .then(id => {
