...
function linkAdvertise(proto) {
  return {
    ...
    setup() {
      ...
      abeagle.getExperimentVariant('advertise_international', {rejectErrors: false})
        .then((variant) => {
          if(variant !== 'on') {
            return;
          }
          this.on(`ad-content-ready-${this.config.wid}`, ({data: {type, buzz}}) => {
            if (type.match(/programmatic|impression_pixel/) || (buzz && /other/.test(buzz.user_type))) { return; }
            this.element.insertAdjacentHTML('beforeend', template.render({ 'slot': this.config }));
