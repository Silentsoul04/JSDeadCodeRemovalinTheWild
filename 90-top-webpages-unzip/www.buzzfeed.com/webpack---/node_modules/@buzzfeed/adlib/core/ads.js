import { getUserId, loadScript } from '@buzzfeed/bf-utils';
import { encode as hashidsEncode } from '@buzzfeed/bf-utils/lib/hashing';
import { queryStringToObject, urlQueryToObject } from '@buzzfeed/bf-utils/lib/query-string';
...
import { EventManager, publicEvents } from '../js/services/events';
import pageTargeting from '../js/services/page-targeting';
import { PrebidInitializer } from '../js/services/prebid';
import prebidUnit from '../js/services/prebid-ad-unit';
import { AmazonInitializer } from '../js/services/amazon';
import gdpr from '../js/services/gdpr';
import { notifyDataDog, notifyError } from '../js/services/datadog'; // @todo Remove the circular reference
import { thumbnailReady } from '../js/services/thumbnail-ready';
...
import { getReferrer } from '../js/services/referrer';
...
const util = { loadScript, queryStringToObject };
...
function _onSlotRenderEnded(event) {
  ...
  try {
    ...
    if (!slot || !wid) {
      window.raven && window.raven.captureMessage('onSlotRenderEnded error', { tags: { gptEvent: event } });
      return;
    }
...
function _onSlotLoad(event) {
  ...
  try {
    ...
    const slot = _getSlotContainer(wid);
    if (!slot || !wid) {
      window.raven && window.raven.captureMessage('onSlotLoad error', { tags: { gptEvent: event } });
      return;
    }
 ...
 */
function _getPPID() {
  return Promise.all([gdpr.hasConsented(), getUserId({ env: BZFD.Config.env })])
...
const ads = {
   ...
   */
  configure({customTargetingPage, customTargetingSlot}) {
  ...
  prepareSlot(options) {
    ...
    if (!_slotStatus[slotId] || !_slotStatus[slotId]['prepared']) {
      _slotStatus[slotId] = {};
      _slotStatus[slotId]['prepared'] = this.init().then(({ prebidServices }) => {
   ...
   */
  buildAdCall(options) {
    let {adPos, adType, zone1} = options;
  ...
  defineSlot(options, slotTargeting = {}) {
    let {wid, size = 'fluid', targeting = {}} = options;
    const googletag = window.googletag;
      ...
      _slotStatus[slotId]['displayed'] = this.init()
        .then(() => {
          ...
          googletag.cmd.push(() => {
            ...
            googletag.pubads().refresh([_slots[wid]], {changeCorrelator: false});
          });
        });
    }
  ...
  forceRefresh(options) {
    ...
    if(!_slotStatus[slotId]['displayed']) {
      ...
      window.googletag.pubads().refresh([_slots[wid]], {changeCorrelator: false});
    }
  },
  ...
  isProgrammaticSlot(gptEv, requestedSize = []) {
    ...
    return sizeUtils.isProgrammatic(size, {strict: false});
  },
 ...
 */
window.BZFD.Config.ads.handleBrandedBuzzData = (wid, { buzzes }) => {
