...
import { Deferred } from '@buzzfeed/buzzblocks/js/services/util/standalone';
...
class DimensionObserver {
  constructor(context) {
    ...
    this.isActive = true;
    this.ad = {type: null, height: 0, duration: 0};
  ...
  init() {
    ...
    this.onmessage = {
      [`ad-content-ready-${this.config.wid}`]: ({type}) => {
      ...
      [`ad-native-video-ready-${wid}`]: ({duration}) => {
