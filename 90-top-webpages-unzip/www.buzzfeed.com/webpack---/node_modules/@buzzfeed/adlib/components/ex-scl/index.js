...
import { lazyRender } from '../../core/mixins/rendering';
import { AdExBase } from '../ex';
import { adHookevents, eventPrefix as hookPrefix } from '../hook-manager';
...
adHookevents.on(`${hookPrefix.CONFIG}ad-ex-scl`, ({data}) => {
  if(!data.slots) {
    return;
  }
...
adHookevents.on(`${hookPrefix.GRABBED}ad-ex-scl`, ({data}) => {
  let { hooks, parent } = data;
  ...
  for(let i = 0; i < hooks.length; i++) {
    ...
    hookEl.insertAdjacentHTML('beforeend', adxTemplate.render({ 'slot': currSlot }));
