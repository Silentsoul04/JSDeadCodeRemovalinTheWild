...
import { loadScript } from '@buzzfeed/bf-utils';
import bzfd from '../../js/services/bzfd';
import { AdFormat } from '../../core';
...
class AdBFPPromo extends AdFormat {
   ...
   */
  buildFormat() {
    const { bfpScript, bfpFormatName } = this.ad;
  ...
  initBFP() {
    ...
    windowBfp.init({
      ...
      config: {
        ...
        context: { experimentVariant: this.experimentVariant }
      },
      broadcast: () => {}
    });
  ...
  setupDFPClickTracker() {
    const { dfpClickTracker, clickThroughUrl } = this.ad;
