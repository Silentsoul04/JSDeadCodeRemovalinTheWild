...
class UnreadIcon extends Component {
  render({ count }) {
    return (
      <div className={styles.unreadIcon}> {count} </div>
    );
  }
}
...
class Notification extends Component {
  ...
  render({ body, triggered_at }) {
    return (
      <li>
        <div className={styles.notification}>
          <div>{body}</div>
          <div>{this.timeAgo(new Date(triggered_at))}</div>
        </div>
      </li>
    );
  }
...
export class Notifications extends Component {
  render({ unreadNotifications = [], readNotifications = []}) {
    return (
      <div className={styles.notifications}>
        <h2 className={styles.notificationsTitle}>Notifications</h2>
        <div className={styles.unreadTitle}>
          <h3>Unread</h3>
          {unreadNotifications.length > 0 ? <UnreadIcon count={unreadNotifications.length} /> : ''}
        </div>
        {
