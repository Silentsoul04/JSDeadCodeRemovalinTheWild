...
const Avatar = ({ image, username, unreadNotificationsCount, ...props }) => (
  <div className={styles.userAvatarContainer}>
    ...
    {unreadNotificationsCount > 0 && (<span className={styles.userAvatarUnread} />)}
  </div>
);

const UserOptions = ({ image, displayName, username, isCommunityUser, onLogoutClicked, onUserMenuClicked, unreadNotifications, readNotifications, isVisible }) => {
...
class NotifUserMenuContainer extends Component {
  ...
  render({ isVisible, userInfo, ...props }) {
    return (
      <section className={`${styles.userMenuContainer}  ${this.props.isVisible ? styles.visible : ''}`} id={userMenuId} ref={container => this.container = container}>
        {
          this.props.isVisible && (
            <FocusTrap focusTrapOptions={{clickOutsideDeactivates: true}}>
              <UserOptions {...userInfo} {...props} isVisible={isVisible} />
            </FocusTrap>
          )
        }
      </section>
...
class NotifUserMenu extends Component {
  ...
  fetchNotifications(userID) {
    return fetch(`${this.props.bfURL}/notification-api/v1/notifications?user_id=${userID}`, {credentials: 'include'})
  ...
  render({ isVisible, userInfo, onLogoutClicked, onUserMenuClicked }) {
    return (
      <div>
        <ThemedToggle
          onClick={onUserMenuClicked}
