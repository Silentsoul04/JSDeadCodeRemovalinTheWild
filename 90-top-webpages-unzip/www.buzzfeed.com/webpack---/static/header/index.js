...
import { PageOverlay } from './more-nav';
import DestinationNav from './destination-nav';
import { I18n, Config, Theme } from './context';
...
export default class Header extends Component {
  ...
  toggleMoreNav({ forceVisible = !this.state.showMoreNav }) {
    this.setState({
      ...this.state,
  ...
  onScroll() {
    ...
    if (this.state.isSticking === nextIsSticking) { return; }
  ...
  render({ navItems, config, i18n, theme, ...props }, state) {
        ...
        <Config.Provider value={config}>
          <Theme.Provider value={theme}>
            <header className={styles.header}>
              ...
              <div className="js-sticky-container" ref={el => this.stickyContainer = el}>
                <ThemedMainNav
                  navItems={navItems}
                  userInfo={state.userInfo}
                  showMoreNav={state.showMoreNav}
                  showNotifMenu={state.showNotifMenu}
              ...
              { state.showMoreNav && <PageOverlay top={this.overlayTopOffset()} onHide={this.toggleMoreNav} /> }
              { state.showNotifMenu && <PageOverlay top={this.overlayTopOffset()} onHide={this.toggleNotifMenu} /> }
            </header>
          </Theme.Provider>
        </Config.Provider>
      </I18n.Provider>
