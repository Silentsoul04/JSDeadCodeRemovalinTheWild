import { defaultsDeep, bind, keys, isString, isFunction } from 'lodash';
...
function GoogleAnalyticsIntegration(sb) {
  ...
  this._trackPage = function(dataObj) {
    ...
    if (keys(mappedEvent).length > 0) {
      if (this.generalSettings.mode === sb.defs.bfaMode.prod) {
        ...
        this.tracker('send', 'pageview', window.location.pathname + pageNumber, {'dimension1': 'feedpager'});
      }
      sb.debugTracker.trackEvent(eventInfo, mappedEvent);
    } else {
  ...
  this._trackPageExit = function(dataObj, routeVars) {
    ...
    this._trackDefault({ l: 'Feed', n: 'last-cell-visible', d: _this.feedIndex, }, 'update/feedindex');
  };
  ...
  this._trackRawEvent = function(dataObj) {
    ...
    const { category, action, label = '(no label)', value = null } = eventInfo.data;
    switch (this.generalSettings.mode) {
    case sb.defs.bfaMode.prod:
    ...
    default:
      ...
      console.table({ category, action, label, value });
      console.groupEnd();
    ...
    init: function() {
      ...
      if (_this.generalSettings.mode === sb.defs.bfaMode.prod) {
        ...
        if (!_this.tracker && !window[_this.settings.extTrackerGlobalName] &&
          ...
          sb.domi.loadScript({src: _this.settings.extFilePath, }, _this.activate, null, _this);
        }
      } else {
        _this.tracker = {};
      }
