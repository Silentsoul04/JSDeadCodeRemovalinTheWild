...
import { name as lazyImage } from '../behaviors/lazy-image';
import { name as viewable } from '@buzzfeed/buzzblocks/js/behaviors/viewable';
import { name as lazyImageBackground } from '../behaviors/lazy-image-background';
...
Application.addModule('package', function(context) {
  ...
  _module.init = function() {
    ...
    if(solid.isXsmall()) {
      dom.find(_element, '.js-xs-impressions').map(i => dom.setAttr(i, {'data-bfa-impressions': 'true'}));
    } else {
      dom.find(_element, '.js-impressions').map(i => dom.setAttr(i, {'data-bfa-impressions': 'true'}));
    }
  };
