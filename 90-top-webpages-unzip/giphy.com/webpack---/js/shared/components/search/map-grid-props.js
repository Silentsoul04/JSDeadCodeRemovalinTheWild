...
import { averageRatio } from 'shared/util/average-gif-size'
import routes, { getRouteFromPathname } from 'shared/routes'

const GiphyTV = loadable(() => import(/* webpackChunkName: "reactGiphyTV" */ '@giphy/react-giphy-tv'))
...
const getGridProps = ({ gifs = [], category, groupId, location, match }) => {
    // use match if available until everything uses the router
    const { params = { term: '' } } = match || getMatch()
    ...
    switch (route) {
        case routes.HOME:
            tvConfig = { ...tvDefaults, aspectRatio: 16 / 9, align: 'center', channel: '@trending' }
            break
        case routes.SEARCH:
            tvConfig = { ...tvDefaults, channel: `#${decodeURIComponent(params.term.replace(/-/g, ' '))}` }
            break
        case routes.CHANNEL:
            // ready for when routing is ready on desktop
            tvConfig = { ...tvDefaults, align: 'left', channel: groupId.split(' - ')[1] }
            break
    ...
    return {
        ...
        tvConfig: tvConfig ? { ...tvConfig, GiphyTV } : null,
        noEditMode,
    }
}

export const mapGroupId = () =>
    withProps(({ groupId, allGifs, match }) => ({
