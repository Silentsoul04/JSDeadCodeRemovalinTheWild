...
// CORPORATE
	...
	mobileRun	:function(){
		...
		var h=g("header");if(h){var bi=h.getElementsByTagName("img")[1],th=h.getElementsByTagName("h1"),tp=h.getElementsByTagName("p"),ft=(th.length)?th:tp,fm=doc.createElement("span"),tt=(bi.alt).split(" - ");fm.innerHTML="<b>"+tt[0]+"</b> "+((tt[1])?(""+tt[1]+""):"");ft[0].appendChild(fm);}
		...
		// // accessibility menu
		if(am){am.innerHTML=am.innerHTML+'<li class="m-link m-form"><a href="#search-form" onclick="return components.mobile.search(this)"><span>'+doc.getElementsByTagName("label")[0].innerHTML+'</span></a></li>';var bc=bo[0].className,lb=am.getElementsByTagName('a');lb[0].innerHTML="<span>"+lb[0].innerHTML+"</span>";lb[0].parentNode.className="m-home";if(me){if(isIE){var o=lb[1].href;lb[1].onmouseover=function(){this.href="javascript://"};lb[1].onmouseout=function(){this.href=o;};lb[1].onclick=function(){components.mobile.menu(lb[1]);}}else{lb[1].setAttribute("onclick","return components.mobile.menu(this);");}lb[1].innerHTML="<span>"+lb[1].innerHTML+"</span>";lb[1].parentNode.className="m-menu";}else{lb[1].parentNode.className="m-hide";}lb[2].parentNode.className="m-hide";}
