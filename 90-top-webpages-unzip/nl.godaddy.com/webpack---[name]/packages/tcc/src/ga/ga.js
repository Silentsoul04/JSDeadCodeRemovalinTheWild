...
function sendPageView(virtualPageName, extras) {
  ...
  const eventObj = gaFilter.getValuesForFilterMap(object.merge(extras, { page_uri: pathname + qs }));
...
function sendEcommerceEvent(type, eventData) {
  ...
  const filterValues = gaFilter.getValuesForFilterMap(object.merge(eventData, { eventtype: type }));
