...
export default class TelephoneInput extends Component {
  constructor() {
    ...
    const { countryKey, phoneNumber, countryData } = props;

    const countryDataProps = (Array.isArray(countryData) && countryData.length &&
      this.sortCountryData(countryData, props.marketId, props.presorted)) || [{
        countryKey: 'US',
        label: 'United States',
        callingCode: '1'
      }];
    ...
    this.getCountryKeyFromCallingCode = this.getCountryKeyFromCallingCode.bind(this);
    ...
    this.normalizeCountryData = this.normalizeCountryData.bind(this);
    this.renderCountryList = this.renderCountryList.bind(this);
    this.sortCountryData = this.sortCountryData.bind(this);
  }
  ...
  componentDidMount() {
    ...
    this.outside('click', this.onOutsideClick, this.phoneControl);
  }
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (lineNumber && callingCode && this.phoneInput.selectionStart < `+${callingCode}.`.length) {
      const phoneNumber = this.state.phoneNumber || this.buildPhoneNumber(callingCode, lineNumber) || '';
      this.phoneInput.selectionStart = this.phoneInput.selectionEnd = phoneNumber.length;
    }
   ...
   */
  onInputFocus() {
    const updated = { showCountryList: false };
  ...
  getSelectedCountry() {
    ...
    const selectedCountry = countryData.filter(country => { return country.countryKey === countryKey; })[0];
    if (!selectedCountry) {
  ...
  render() {
    ...
    return (
      <div className='form-group' ref={ (control) => { this.phoneControl = control; } } >
        <div className='ux-tel-container'>
          <Button design='base' className='ux-tel-btn' id='basic-addon1' onClick={ this.onCountryClick }>
            <span className='country-id'>{ selectedCountry.countryKey } +{ selectedCountry.callingCode }</span>
            <span className='uxicon uxicon-drop-down'></span>
          </Button>
          <input className='form-control ctHidden ux-tel-input'
            name={ name }
            id={ id }
            maxLength={ maxLength }
...
// WEBPACK FOOTER //
// ./node_modules/@ux/telephone-input/index.js