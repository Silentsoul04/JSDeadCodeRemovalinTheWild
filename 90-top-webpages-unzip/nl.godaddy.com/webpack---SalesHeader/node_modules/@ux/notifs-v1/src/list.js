  ...
  render() {
    ...
    return (
      ...
      <div className={ classes } onClick={ stopPropagation }>
        <button className={ closeIconClasses } onClick={ closeList }></button>
        <h4 className={ this.namespace('notifications-list-header') }>
          { messages.headerText }
        </h4>
        <ul className={ this.namespace('notifications-list-items') }>
          { notifications && notifications.length > 0 ? notifications.map((n, c) => {
            return <NotificationListItem key={ c } notification={ n } />;
          })
            : <NotificationsEmpty failed={ failed } messages={ messages } sso={ sso } />
          }
        </ul>
      </div>
    );
