...
class EmailForm extends NamespaceConsumer {
  ...
  render() {
    ...
    if (signingUp) {
      submitMessage = <Spinner size='sm' shade='light'/>;
      ...
      submitMessage = <Icon name={ signUpResult } />;
    } else {
      submitMessage = <FormattedMessage id={ seechange ?
        'Sales:Footer:SpecialOfferForm:SignUp' :
        'Sales:Footer:SpecialOfferForm:SubmitButton' } />;
    }

    const placeholder = props.messages['Sales:Footer:SpecialOfferForm:EmailPlaceholder'];

    return (
      <div className={ this.namespace('newsletter-form') }>
        <label htmlFor='specialOfferForm'>
          <FormattedMessage id={ seechange ?
            'Sales:Footer:SpecialOfferForm:Label' :
            'Sales:Footer:SpecialOfferForm:Heading' } />
        </label>
        <form onSubmit={ this.submitEmailForm }>
          <div className={ this.namespace('input-group') }>
            <input
              id='specialOfferForm'
              className={ this.namespace(...emailClasses) }
            ...
            <div className={ this.namespace('input-group-btn') }>
              <Button type='submit' design={ buttonColor } onClick={ this.submitEmailForm }>
                { submitMessage }
              </Button>
            </div>
          </div>
          { errorMessage &&
            <p aria-live='polite' className={ this.namespace(...errorState) }>
