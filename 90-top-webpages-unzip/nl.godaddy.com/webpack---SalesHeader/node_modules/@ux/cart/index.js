...
export class Cart extends NamespaceConsumer {
  ...
  render() {
    ...
    const icon = (<span
      aria-label={ props.messages[messageId] }
      className={ this.namespace(
        'app-icon',
        'app-icon-cart',
        this.classNames({ 'cart-empty': !items })) } />);
    ...
    if (props.textOnly) {
      return (
        <FormattedMessage id='Shared:UtilityBar:Cart'>
          { stringLiteral => <Link ariaLabel={ stringLiteral }
            urlArgs={ props.urlArgs }
            href={ props.checkout }
            data-eid={ props.eid }
            className={ this.classNames({ [this.namespace('cart-empty')]: !items }, props.className) }>
            { stringLiteral }
          </Link> }
        </FormattedMessage>
      );
    ...
    if ((props.showEmpty && !items) || this.isMobile) {
      return (
        <Link
          ariaLabel={ props.messages[messageId] }
          urlArgs={ props.urlArgs }
          href={ props.checkout }
          data-eid={ props.eid }
    ...
    return (
      <IntlProvider locale={ props.market } messages={ props.messages }>
        <Link urlArgs={ props.urlArgs } href={ props.checkout } data-eid={ props.eid } className={ props.className }>
          { icon }
