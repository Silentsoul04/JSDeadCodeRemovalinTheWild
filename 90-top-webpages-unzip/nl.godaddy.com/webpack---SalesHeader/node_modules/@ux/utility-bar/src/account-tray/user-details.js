      ...
      return (
        <li>
          <Link urlArgs={ props.urlArgs }
            { ...props.urls.pro }
            data-eid={ eids.proDashboard }>
            <FormattedMessage id={ golf.proDashboard } />
          </Link>
        </li>
      );
    }
  ...
  render() {
    ...
    return (
      <IntlProvider locale={ props.market } messages={ props.messages }>
        <div className={ this.namespace('account-info', 'collapse', 'in') }>
          <TrayTitle>
            <Link urlArgs={ props.urlArgs }
              { ...props.urls.products }
              data-eid={ eids.manageAccount }
              className={ `${this.namespace('font-primary-bold', 'name')} ctHidden` }>
              { /* props.name is the full name (e.g. "Jane Doe") */ }
              { props.name }
            </Link>
          </TrayTitle>

          <div className={ this.namespace('customer-number') }>
            <strong><FormattedMessage id={ golf.shopperid } /></strong>
            <span className={ `${this.namespace('shopper-id')} ctHidden` }>{ props.shopperId } | </span>
            <strong><FormattedMessage id={ golf.pinLabel } /></strong>
            <Link urlArgs={ props.urlArgs }
              { ...props.urls.viewPin }
              data-eid={ eids.viewPin }
              className={ `${this.namespace('customer-pin')} ctHidden` }>
              <FormattedMessage id={ golf.viewPin } />
            </Link>
          </div>

          <Vip details={ props.vip } market={ props.market } messages={ props.messages } />

          <ul className={ this.namespace('list-unstyled', 'quick-links') }>
            { this.renderPro() }
            { this.renderMyAccountHome() }
            <li>
              <Link urlArgs={ props.urlArgs }
                { ...props.urls.products } data-eid={ eids.products }>
                <FormattedMessage id={ golf.products } />
              </Link>
            </li>
            <li>
              <Link urlArgs={ props.urlArgs }
                { ...props.urls.account } data-eid={ eids.accountSettings }>
                <FormattedMessage id={ golf.accountSettings } />
              </Link>
            </li>
            <li>
              <Link urlArgs={ props.urlArgs } { ...props.urls.renewals } data-eid={ eids.renewals }>
                <FormattedMessage id={ golf.renewals } />
              </Link>
            </li>
          </ul>

          <Link urlArgs={ props.urlArgs } { ...props.urls.products }
            data-eid={ eids.visitMyAccount }
            className={ this.namespace('btn', 'btn-purchase') }>
            <FormattedMessage id={ golf.visitMyAccount } />
          </Link>
          <Link urlArgs={ props.urlArgs } { ...props.urls.sso.logout }
            data-eid={ eids.logout }
            className={ this.namespace('logout-link') }>
            <FormattedMessage id={ golf.logout } />
          </Link>
        </div>
      </IntlProvider>
    );
