...
import { request, cookie, URL } from '@ux/util';
...
class CurrencySelector extends NamespaceConsumer {
  ...
  fetchCurrencies(done) {
    ...
    const { market, privateLabelId } = this.props;
    this.markets.getCurrencies(gui, market, privateLabelId, (err, results) => {
      ...
      results = (results || []).map(cur => {
        ...
        return { code: cur.currencytype, symbol: cur.symbol };
      });
   ...
   */
  render() {
    const { currencies, priorityCurrencies, selected } = this.state;
    const { eidPrefix, messages, market } = this.props;
    ...
    return (
      <IntlProvider locale={ market } messages={ messages }>
        <FormattedMessage id='Sales:Footer:CurrencySelectorTray:AriaLabel' values={{ selection: selected }}>
          {(currencySelectorTray) =>
            <TrayMenu ariaLabel={ currencySelectorTray }
              name={ selected }
              type='base'
              className={ trayMenuClassNames }
              ref={ c => this.tray = c }
              data-eid={ eid }>
              {
                priorityCurrencies.map((cur, i) => {
                  ...
                  return <CurrencyItem
                    ...
                    eidPrefix={ eidPrefix }
                    onClick={ this.onCurrencyChanged.bind(this, cur) } />;
                })
              }

              <div className={ this.namespace('divider') }></div>
...
class CurrencyItem extends React.Component {
   ...
   */
  onClick(evt) {
    const { code, symbol } = this.props;
    this.props.onClick(evt, { code, symbol });
  }
   ...
   */
  render() {
    const { code, symbol, eidPrefix } = this.props;
    ...
    return (
      <button type='button' className={ this.props.className } onClick={ this.onClick } data-eid={ eid }>
        <span>{ code }</span> <span>{ symbol }</span>
      </button>
    );
  }
}
