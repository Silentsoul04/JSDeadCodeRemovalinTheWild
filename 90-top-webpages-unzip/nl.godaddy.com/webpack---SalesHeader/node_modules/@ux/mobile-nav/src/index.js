...
export class MobileNav extends NamespaceConsumer {
  ...
  renderCaption(caption, isLiteral = false) {
    ...
    return <FormattedHTMLMessage id={ caption } />;
  ...
  renderIcon(icon) {
    if (icon) {
      return <span className={ this.namespace(icon, 'uxicon', 'custom-icon') }/>;
    }
    return null;
  }
 ...
 */
  renderPlainButton(item, idx, eid, active) {
    const { urlArgs, useGradientHighlightBar } = this.props;
    const {
      caption,
      title,
      icon,
    ...
    const plainButton = (
      <Link key={ (caption || id) + idx }
        className={ this.namespace(
          this.classNames('column-heading', { active })) }
        href={ href }
        urlArgs={ urlArgs }
        data-eid={ eid }
        target={ target }
        onClick={ onClick && this.onItemClick.bind(this, item) }
    ...
    return (<div className={ this.namespace('column-container') }>
      { active && <div className={ this.namespace('column-highlight') } role='presentation'>&nbsp;</div> }
      { plainButton }
    </div>);
  }
 ...
 */
  renderHeadingWithChildren(item, idx, eid, active) {
    const { caption, title, id } = item;

    const menuItem = (
      <div key={ (caption || id) + idx }
        className={ this.namespace(
          this.classNames('column-heading', { active })) }
        onClick={ this.onHeadingClick.bind(this, item) }
        data-eid={ eid }>
        { this.renderCaption(title, true) || this.renderCaption(caption) }
        { this.renderIcon(item.icon) }
        <span className={ this.namespace('uxicon-chevron-right-lt', 'uxicon') }/>
      </div>
    );
    ...
    return props.nav.map((item, idx) => {
      ...
      if (caption || title) {
        ...
        if (props.growthnav2 && id === 'Sales:ProductNav:PartnerPrograms') {
          ...
          return (<React.Fragment key={ id + idx }>
            <hr/>
            { content }
          </React.Fragment>);
        }
  ...
  renderTopContainer(showBackBtn) {
    const { props } = this;
    return (
      <div className={ this.namespace('mobile-top-container') }>
        { showBackBtn &&
        ...
        { !showBackBtn && props.showMenuTitleOnHome &&
          <div className={ this.namespace('mobile-top-container-label', 'font-primary-bold') }>
            { this.renderCaption(this.props.menuTitle) }
          </div>
        }
        <span onClick={ this.close } className={ this.namespace('uxicon-x', 'uxicon', 'pull-right') }/>
      </div>
    );
  }
  ...
  renderSubNavPanels() {
    ...
    const { caption, title } = selectedItem || {};
    props.nav.forEach((navItem, i) => {
      if (navItem.children) {
        ...
        panels.push(<SubNavPanel
          onClose={ this.close }
          onBack={ this.goToMainMenu }
          open={ open }
          navigation={ navItem }
          menuTitle={ props.menuTitle }
          eidPrefix={ props.eidPrefix }
          literalCaptions={ props.literalCaptions }
          currentHref={ props.currentHref }
          urlArgs={ props.urlArgs }
          key={ 'subNavPanel' + i }
          ref={ (r) => { if (open) { this.subNavPanel = r; } } }
        />);
      }
    });
  ...
  renderHeadingsPanel() {
    ...
    return (
      <div className={ this.namespace(
        ...
        <div className={ this.namespace('mobile-wrap') }>
          <div className={ this.namespace('mobile-label', 'font-primary-bold') }>
  ...
  renderButtons() {
    ...
    return (
      <ul className={ this.namespace(
        ...
        <li>
          <Link ariaLabel={ leftA11y } href={ leftHref || '#' } data-eid={ leftEid }>
            <span className={ this.classNames(this.namespace('uxicon'), leftIcon) } aria-hidden='true'></span>
          </Link>
        </li>
        <li>
          <Link ariaLabel={ props.messages[golf.myAccount] }
            href={ props.urls.account && props.urls.account.href || '#' }
            data-eid={ eids.myAccount } >
            <span className={ this.namespace('uxicon', 'uxicon-user') } aria-hidden='true'></span>
          </Link>
        </li>
        <li>
          <Link ariaLabel={ props.messages[golf.help] }
            href={ props.urls.help && props.urls.help.href || '#' }
            data-eid={ eids.help }>
            <span className={ this.namespace('uxicon', 'uxicon-help') } aria-hidden='true'></span>
          </Link>
        </li>
      </ul>
    );
  ...
  render() {
    ...
    const trayName = (
      <span className={ this.namespace('mobile-tray-name') }>
        { buttonTitle }
      </span>
    );
    ...
    return (
      <IntlProvider locale={ props.market } messages={ props.messages }>
        <div className={ this.classNames(props.className, this.namespace('mobile-top-menu-item')) }>
          <FormattedMessage id={ props.menuTitle }>
            {text => (
              <button
                onClick={ this.toggleOpen }
                className={ this.namespace('nav-container', 'btn') }
                aria-label={ text }
                data-eid={ props.menuButtonEid }
              >
          ...
          onScroll={ this.killRunway } >
            ...
            <div className={ this.namespace(this.classNames(
              'runway-container',
              { show: state.runway })) }>
              <span className={ this.namespace('uxicon', 'uxicon-chevron-down-lt') } />
              <span className={ this.namespace('uxicon', 'uxicon-chevron-down-lt') } />
              <span className={ this.namespace('uxicon', 'uxicon-chevron-down-lt') } />
            </div>
          </div>
