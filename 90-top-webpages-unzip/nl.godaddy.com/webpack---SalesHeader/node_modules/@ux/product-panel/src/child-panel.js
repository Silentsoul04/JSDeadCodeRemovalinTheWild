...
class ChildPanel extends NamespaceConsumer {
  ...
  renderBackBtn() {
    return (
      <div className={ this.namespace('child-flyout-back') }>
        <button onClick={ this.props.onBack } className={ this.namespace('btn') }>
          <FormattedHTMLMessage id={ golf.mainMenu } />
        </button>
      </div>
    );
  }
  ...
  renderTitleAndDescription() {
    ...
    return (
      <div className={ this.namespace('child-panel-top-info') }>
        { text && <div className={ this.namespace('product-flyout-heading') }>
          { this.renderTitle(text, !caption, leftnav.href, props.urlArgs) }
        </div> }
        { description && <p className={ this.namespace('description') }>
  ...
  renderLinks(items) {
    ...
    items.forEach((child, i) => {
      ...
      if (href && href.length && text) {
        ...
        links.push(
          <li key={ id || (caption + i) } className={ this.namespace('child-menu-link') }>
            <Link
              data-eid={ eid }
              href={ href }
              urlArgs={ props.urlArgs }
  ...
  renderRows(children, numCols) {
    ...
    return rows.map((colArr, i) => {
      ...
      return <div key={ 'row' + i } className={ this.namespace('child-menu-items-row') }>
        { colArr.map((frag, j) => {
          return <div key={ j } className={ this.namespace('child-menu-items-col') }>
            { frag }
          </div>;
        }) }
      </div>;
  ...
  renderSections(sections) {
    ...
    sections.forEach((section, i) => {
      ...
      if (Array.isArray(children) && children.length > 0) {
        ...
        subNavSections.push(
          <React.Fragment key={ section.id || (caption + i) }>
            { showSectionHeader && <div className={ this.namespace('child-menu-header', 'title', 'small') }>
              { renderCaption(caption || title, !caption) }
            </div> }
            <ul className={ this.namespace('child-menu-links', 'list-unstyled') }>
  ...
  render() {
    ...
    return (
      <div className={ this.namespace(
        ...
        { this.renderBackBtn() }
        <div className={ this.namespace('child-menu-container', 'menu-scroller') }
          onScroll={ this.props.onScroll }
          ref={ (r) => { this.outerDiv = r; } }>
          <div ref={ (r) => { this.innerDiv = r; } }>
