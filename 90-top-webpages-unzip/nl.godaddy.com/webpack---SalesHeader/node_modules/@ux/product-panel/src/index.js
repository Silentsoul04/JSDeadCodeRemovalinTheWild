...
export class ProductPanel extends NamespaceConsumer {
  ...
  renderChildPanels(nav) {
    ...
    const { caption = null, title = null } = selectedParent || {};
    nav.forEach((navItem) => {
      if (navItem.children) {
        ...
        panels.push(<ChildPanel
          parent={ navItem }
          onClose={ this.close }
          onBack={ this.goToParentMenu }
          open={ open }
          navigation={ navItem.children }
          eidPrefix={ props.eidPrefix }
          key={ navItem.caption || navItem.id }
          urlArgs={ props.urlArgs }
          messages={ props.messages }
          ref={ (r) => { if (open) { this.childPanel = r; } } }
  ...
  renderPanelFooter() {
    ...
    return (
      <div className={ this.namespace('product-flyout-footer') }>
        <div className={ this.namespace('container-fluid') }>
          <div className={ this.namespace('row') }>
            <div className={ this.namespace('col-md-7', 'product-flyout-legal') }>
            ...
            <div className={ this.namespace('col-md-5', 'product-flyout-selectors-wrap') }>
              { props.cornerContent }
            </div>
          </div>
        </div>
  ...
  renderGlobalElements() {
    const {
      ...
      loggedIn,
      urlArgs,
      globalElements } = this.props;

    if (!globalElements) { return null; }
  ...
  render() {
    ...
    return (
      <IntlProvider locale={ props.market } messages={ props.messages }>
        <React.Fragment>
          <div className={ this.namespace(this.classNames('product-flyout-btn-wrap', { 'is-open': state.open })) }>
          ...
          <div className={ this.namespace(
            this.classNames('product-flyout-menu', { open: state.open })) }>
            { this.renderGlobalElements() }
            <FormattedMessage id={ golf.close }>
              {(closeTxt) =>
            ...
            <ParentPanel onClose={ this.close }
...
ProductPanel.defaultProps = {
  ...
  market: 'en-US',
  control: <Icon name='grid-horizontal' size={ 18 } />
};
