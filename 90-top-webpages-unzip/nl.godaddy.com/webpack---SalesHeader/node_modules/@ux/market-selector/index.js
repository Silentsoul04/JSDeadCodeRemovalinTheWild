...
export class MarketSelector extends NamespaceConsumer {
  ...
  renderMarkets() {
    ...
    return markets.reduce((acc, market) => {
      ...
      if (market) {
        ...
        acc.push(
          <ColumnItem key={ marketName } isSelected={ marketName === currentMarket }>
            { /* We use the `redirect` feature in <Link /> to redirect to a different */ }
            { /* href on click than that which is displayed to the user. */ }
            <Link { ...url }
              data-tcc-ignore
              urlArgs={ urlArgs }
              redirect={ isReseller ? null : redirect.href }
              data-market={ marketName }
              data-eid={ eid }>
              <span data-tcc-ignore>
                <strong data-tcc-ignore>{ market.countryName }</strong> - { market.languageName }
              </span>
            </Link>
          </ColumnItem>
        );
  ...
  render() {
    ...
    return (
      <IntlProvider locale={ currentMarket } messages={ messages }>
        <FormattedMessage id='Shared:UtilityBar:MarketSelectorTray:AriaLabel' values={{ selection: marketDescription }}>
         {(marketSelectorTray) =>
          <TrayMenu ariaLabel={ marketSelectorTray } type='base'
            ref={ c => this.trayMenu = c } // eslint-disable-line no-return-assign
            ...
            <div className={ this.namespace('container') }>
              ...
              <Columns num={ numColumns } style='vertical' className={ this.namespace('market-list') }>
                { this.renderMarkets() }
              </Columns>
            </div>
          </TrayMenu> }
