...
!(function ($) {
    ...
    */
            ...
            <ul class="btns">\
              ...
              <a target="_blank" class="" href="//i.csdn.net/#/msg/index"><img src="https://csdnimg.cn/public/common/toolbar/images/messageIcon.png" alt="" class="message"><div class="toolbar-circle" id="msg-circle"></div></a>\
                <div class="msgList" id="msgList">\
                  <dd><a href="//blogdev.blog.csdn.net/article/details/101446074" id="msgList-notice">公告<em class="notice"></em></a></dd>\
    ...
    function getNotice (){
        // 公告提示
        // 判断是否点击
        let isNotice = getCookie("notice")
        if (!isNotice) {
            let msgListNotice = $('#msgList-notice')
            let msgCircle = $('#msg-circle')
            let numBrfore = msgCircle.text()
            let numAfter = parseInt(numBrfore) ? parseInt(numBrfore) + 1 : 1
            msgCircle.text(numAfter).fadeIn()
            msgListNotice.find('em').text(1)
            msgListNotice.click(function(){
                setCookie('notice', 1)
                msgListNotice.find('em').empty()
                msgCircle.text(numBrfore)
            })
        }
    }
    /*
