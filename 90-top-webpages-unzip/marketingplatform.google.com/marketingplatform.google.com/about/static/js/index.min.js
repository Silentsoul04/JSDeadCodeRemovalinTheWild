...
attributeFilter:["class"]};for(k=0;k<a.length;k++)(new MutationObserver(e)).observe(a[k],c);document.querySelectorAll("main.gmp-page article.gmp-section a:not([data-g-action])").forEach(function(a){a.setAttribute("target","_blank");a.addEventListener("click",function(a){window.dataLayer.push({eventCategory:window.dataCategory,eventAction:"link: "+a.target.textContent.toLowerCase().replace(/[\-/]/g," ").replace(/%/g," percent").replace(/\&/g,"and").replace(/@/g,"at").replace(/[\\\^\$\*\?\.\(\)\|\{\}\[\]!,\u201c\u201d<>#\u2019"']/g,
"").trim(),eventLabel:window.dataLabel,event:"trackEvent"})})});document.querySelectorAll("form").forEach(function(a){a.addEventListener("change",function(a){a=a.target;if("checkbox"!==a.type||a.checked)window.dataLayer.push({event:"trackEvent",eventCategory:a.getAttribute("data-g-event"),eventAction:a.getAttribute("data-g-action"),eventLabel:a.getAttribute("data-g-label")});else return!1})});document.querySelectorAll('[data-g-event][data-g-action]:not(input):not(select):not(textarea):not([type="submit"])').forEach(function(a){a.addEventListener("click",
function(){window.dataLayer.push({eventCategory:a.getAttribute("data-g-event"),eventAction:a.getAttribute("data-g-action"),eventLabel:a.getAttribute("data-g-label"),event:"trackEvent"})})})}Y.prototype.updateTree=function(a){this.decisionTree.submitUrl=null;this.decisionTree.nextLeaf=null;-1<a.indexOf("leaf_")?this.decisionTree.nextLeaf=a:0!==a.indexOf("step_")&&(this.decisionTree.submitUrl=a,delete this.treeFormData.step_2)};
...
!1):(a.classList.add("is-visible"),a.setAttribute("aria-hidden",!1))};Y.prototype.handleBackgroundClick=function(a,c){a.target.id===c&&this.toggleDecisionTreeVisibility(c)};Y.prototype.goBack=function(){window.history.back()};Y.$inject=["$scope","$window","$timeout"];var ja={attributes:!0,childList:!1,subtree:!0,attributeOldValue:!0,attributeFilter:["class"]};
...
a+'"]');return ia(a)}function m(a){a=h[a].$$attr.fbxId;return"undefined"!==typeof d.formData[a]&&""!==d.formData[a]}function v(){c(function(){var a=ia(h.$$element[0].querySelectorAll(".gmp-formfield__error:not(.ng-hide)"));a.length&&"function"===typeof a[0].scrollIntoView&&a[0].scrollIntoView({behavior:"smooth"})},270)}function B(a){return h.$$element[0].querySelector(".gmp-formfield--floating-label[data-field-name="+a+"]")}d.setFocus=function(a){B(a).classList.add("is-focused")};d.unsetFocus=function(a){B(a).classList.remove("is-focused")};
d.handleChange=function(a){var c=B(a);c&&(m(a)?c.classList.add("is-dirty"):c.classList.remove("is-dirty"));h[a].$setValidity("valid",!0);delete h[a].$error.invalid};d.toggleAllChoices=function(a){var c=h[a].$$attr.fbxId;d.handleChange(a);var e=d.formData[c].split(",").filter(function(a){return""!==a});a=l(c);var g=[],k=!0;e.length>=a.length&&(k=!1);for(e=0;e<a.length;e++)k&&g.push(a[e].value),a[e].checked=k;d.formData[c]=g.join(",")};d.areAllChoicesSelected=function(a){return!l(h[a].$$attr.fbxId).some(function(a){return!a.checked})};
