(function() {
    if (!PluginDetect) var PluginDetect = {
        ...
        findNavPlugin: function(g, d) {
            var a =
                g.constructor == String ? g : g.join(".*"),
                e = d === false ? "" : "\\d",
                b, c = new RegExp(a + ".*" + e + "|" + e + ".*" + a, "i"),
                f = navigator.plugins;
            for (b = 0; b < f.length; b++)
                if (c.test(f[b].description) || c.test(f[b].name)) return f[b];
            return null
        },
        ...
        isActiveXObject: function(b) {
            var d = this,
                a, g, f = "/",
                c = '<object width="1" height="1" style="display:none" ' + d.plugin.getCodeBaseVersion(b) + ">" + d.plugin.HTML + "<" + f + "object>";
    ...
    PluginDetect.shockwave = {
        ...
        getVersion: function() {
            ...
            if (!c.isIE) {
                ...
                if (typeof b == "string" && b.length > 0) a = c.getNum(b);
    ...
    PluginDetect.instantiate = function(h, b, c, a) {
        ...
        var k, l = document,
            g = this,
            p, i = l.getElementsByTagName("body")[0] || l.body,
        ...
        if (!g.div) {
            ...
            if (f) {
                ...
                if (i.firstChild && typeof i.insertBefore != "undefined") i.insertBefore(g.div, i.firstChild);
            ...
            if (!e.isIE) {
                if (e.hasMimeType(this.mimeType)) {
                    if (e.findNavPlugin(["Windows",
                            "Media", "(Plug-in|Plugin)"
                        ], false) || e.findNavPlugin(["Flip4Mac", "Windows", "Media"], false)) this.installed = 0;
                    var d = e.isGecko && e.compareNums(e.GeckoRV, e.formatNum("1.8")) < 0;
                    if (!d && e.findNavPlugin(["Windows", "Media", "Firefox Plugin"], false)) {
                        var c = e.instantiate("object", ["type", this.mimeType[0]], []),
                            f = e.getObject(c);
    ...
    PluginDetect.silverlight = {
        ...
        getVersion: function() {
            ...
            if (!c.isIE) {
                var a = [null, null],
                    e = c.findNavPlugin("Silverlight Plug-in", false),
    ...
    function md5(source) {
        ...
        BaiduWebAdapter.prototype.initPlugin =
            function(silence, callback) {
                ...
                var me = this;
                if (!this._curPlugin) {
                    for (i = 0, len = this._pluginList.length; i < len; i++)
                        if (this._pluginList[i].isSupported()) {
        ...
        BaiduWebAdapter.prototype.launchApp =
            function(appName, installAppName, param, requiredAdmin, callback) {
                if (this._curPlugin) this._curPlugin.launchApp(appName, installAppName, param, requiredAdmin, callback);
                else this._sendCallback(callback, -1)
            };
        BaiduWebAdapter.prototype.queryAppVersion = function(appName, callback) {
            if (this._curPlugin) this._curPlugin.queryAppVersion(appName, callback);
        ...
        BaiduWebAdapter.prototype.setParam = function(key, value, callback) {
            if (this._curPlugin) this._curPlugin.setParam(key, value, callback);
            else this._sendCallback(callback, -1)
        };
        BaiduWebAdapter.prototype.getParam = function(key, callback) {
            if (this._curPlugin) this._curPlugin.getParam(key, callback);
            else this._sendCallback(callback, "")
        };
        BaiduWebAdapter.prototype.queryGuid = function(callback) {
            if (this._curPlugin) this._curPlugin.queryGuid(callback);
            else this._sendCallback(callback,
                "")
        };
        BaiduWebAdapter.prototype.getSysInfo = function(callback) {
            if (this._curPlugin) this._curPlugin.getSysInfo(callback);
            else this._sendCallback(callback, -1)
        };
        ...
        if (typeof module === "object" && typeof module.exports === "object") module.exports = new BaiduWebAdapter;
        else if (typeof define === "function" && define.amd) define("baiduWebAdapter", [], function() {
    ...
    (function(factory) {
        if (typeof define ===
            "function" && define.amd) define(["baiduWebAdapter"], factory);
        else if (typeof module === "object" && typeof module.exports === "object") module.exports = factory(require("baiduWebAdapter"));
        else factory(window.baiduWebAdapter)
        ...
        BaiduWebAdapter_ActiveX.prototype.initPlugin = function(silence, callback) {
            ...
            if (this.isSupported()) {
                ...
                if (this._isInited) this._sendCallback(callback, 0);
                else this._sendCallback(callback, -1)
            } else this._sendCallback(callback, -1)
        };
    ...
    (function(factory) {
        if (typeof define === "function" && define.amd) define(["baiduWebAdapter"], factory);
        else if (typeof module === "object" && typeof module.exports === "object") module.exports = factory(require("baiduWebAdapter"));
        else factory(window.baiduWebAdapter)
    })(function(baiduWebAdapter) {
        ...
        BaiduWebAdapter_NpApi.prototype._ifUnblocked = function(unblockedCallback, elseCallback) {
            if (this._object) {
                ...
                if (!this.isBlocked) unblockedCallback.call(this);
        ...
        BaiduWebAdapter_NpApi.prototype.initPlugin = function(silence, callback) {
            ...
            if (this.isSupported()) {
                if (silence)
                    if (/Chrome.(\d+\.\d+)/.test(navigator.userAgent) && this._isOriginalChrome() || /Firefox.(\d+\.\d+)/.test(navigator.userAgent)) {
    ...
    function getBrowser() {
        ...
        if (ie) {
            ...
            if (IEVersion < 10 && IEVersion > 6) result = 1;
            else if (IEVersion === 6) result = 6;
            else if (IEVersion > 9) result = 6
        } else if (nav.appName === "Netscape" && /Trident/.test(nav.userAgent)) result =
            7;
        else if (/firefox\/(\d+\.\d)/i.test(ua)) result = 3;
        else if (/chrome\/(\d+\.\d)/i.test(ua)) result = 2;
        else if (/(\d+\.\d)?(?:\.\d)?\s+safari\/?(\d+\.\d+)?/i.test(ua)) result = 4;
    ...
    var fontsStr = "";
    var br = getBrowser();
    var ECLICK_HOST = "//eclick.baidu.com/";
    var logUrl =
        ECLICK_HOST + "fp.htm?";
    ...
    var fpParams = [{
        ...
        value: function() {
            ...
            function getIEPluginsString() {
                ...
                if (activeXFeature()) {
                    var names = ["RvjdlUjnf", "EfwbmWS", "Tipdlxbwf", "Gmbti", "XjoepxtNfejbqmbzfs", "Tjmwfsmjhiu", "WMD", {
                        ...
                        key: "Hpphmf+PofDmjdlDusm",
                        fun: ""
                    }, {
                        key: "RwpeJotfsu+RwpeDusm",
                        fun: "+Wfstjpo"
                    }];
            ...
            if (br === 1) return getIEPluginsString();
            else return getNewChromePluginsString()
        }
    }, {
        key: "sc",
        value: function() {
    ...
    var chromeParams = [{
        key: "ah",
        value: scr["availHeight"]
    }, {
        key: "aw",
        value: scr["availWidth"]
    }, {
        key: "cav",
        value: function() {
            try {
    ...
    var logParams = {
        ci: getCookieRaw("CPROID") || "",
        bi: getCookieRaw("BAIDUID") || ""
    };
    ...
    function createFontsFlash(lastClock) {
        ...
        if (checkFlash()) {
            ...
            flashDom.innerHTML = '<embed wmode="transparent" name="' + fname + '" id="' + fname + '" src="o.swf?v=1" swliveconnect="true" ' + 'quality="high" width="1" height="1" align="middle" allowScriptAccess="samedomain" ' + 'type="application/x-shockwave-flash" pluginspage="//www.macromedia.com/go/getflashplayer">';
            netStatusCheck()
        }
    ...
    function netStatusCheck() {
        ...
        dom.innerHTML = '<embed wmode="transparent" name="nsc" id="nsc"' + ' src="//cpro.baidustatic.com/cpro/ui/nsc.swf" swliveconnect="true" ' + 'quality="high" width="1" height="1" align="middle" allowScriptAccess="samedomain" ' + 'type="application/x-shockwave-flash" pluginspage="//www.macromedia.com/go/getflashplayer">';
    ...
    function init() {
        if (unionFlow)
            if (br === 1 || br === 6 || br === 7) mode = 1;
            else {
                if (br === 2) mode = 2
            }
        else if (br === 1 || br === 6 || br === 7) mode = 3;
        else if (br === 2) mode = 4;
        if (mode !==
            0) {
    ...
    function getChromeChildVersion() {
        ...
        if (/chrome\/(\d+\.\d)/i.test(ua)) {
            if (/SE 2.X MetaSr/i.test(ua)) chromeSubVer = 201;
            else if (/BIDUBrowser/i.test(ua)) chromeSubVer = 202;
            else if (/QQBrowser/i.test(ua)) chromeSubVer = 203;
            else if (/360EE/i.test(ua)) chromeSubVer = 205;
            else if (/QIHU/i.test(ua)) chromeSubVer = 205;
            else if (/2345chrome/i.test(ua)) chromeSubVer =
                206;
            else if (/TaoBrowser/i.test(ua)) chromeSubVer = 208;
            else if (/TheWorld/i.test(ua)) chromeSubVer = 209;
            else if (/Maxthon/i.test(ua)) chromeSubVer = 210;
            else if (/YYE/i.test(ua)) chromeSubVer = 211;
            if (chromeSubVer === 0) {
                var plugin = nav.plugins["Shockwave Flash"];
    ...
    function send() {
        ...
        if (mode === 2 || mode === 4) {
            ...
            log("//eclick.baidu.com/fp.htm?" + paramList.join("&") +
                "&de=ios&", {}, true)
        } else {
