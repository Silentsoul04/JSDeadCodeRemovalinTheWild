(function(e) {
    ...
    t._listener = function(e) {
        var r, n, o, i, s, a, l;
        r = e.origin === "null" ? "file://" : e.origin;
        if (e.data === "cross-storage:poll") {
            return window.parent.postMessage("cross-storage:ready", e.origin)
        }
        if (e.data === "cross-storage:ready") return;
        try {
            o = JSON.parse(e.data)
        } catch (e) {
            return
        }
        if (!o || typeof o.method !== "string") {
            return
        }
        i = o.method.split("cross-storage:")[1];
        if (!i) {
            return
        } else if (!t._permitted(r, i)) {
            s = "Invalid permissions for " + i
        } else {
            try {
                a = t["_" + i](o.params)
            } catch (e) {
    ...
    t._permitted = function(e, r) {
        var n, o, i, s;
        n = ["get", "set", "del", "clear", "getKeys"];
        if (!t._inArray(r, n)) {
            return false
        }
        for (o = 0; o < t._permissions.length; o++) {
            i = t._permissions[o];
            if (!(i.origin instanceof RegExp) || !(i.allow instanceof Array)) {
                continue
            }
            s = i.origin.test(e);
            if (s && t._inArray(r, i.allow)) {
                return true
            }
        }
        return false
    ...
    t._del = function(e) {
        for (var t = 0; t < e.keys.length; t++) {
            window.localStorage.removeItem(e.keys[t])
        }
    };
    t._clear = function() {
        window.localStorage.clear()
    };
    t._getKeys = function(e) {
        var t, r, n;
        n = [];
        r = window.localStorage.length;
        for (t = 0; t < r; t++) {
            n.push(window.localStorage.key(t))
        }
        return n
    };
