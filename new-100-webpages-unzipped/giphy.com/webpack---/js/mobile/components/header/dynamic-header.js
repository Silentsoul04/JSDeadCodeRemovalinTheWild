...
} from '../trending-stories/day/context'
import withWindowScroll, {
    DIRECTION
} from 'shared/components/gif-grid/with-window-scroll'
import {
    getAppSize
...
@connect(state => ({
    appWidth: getAppSize(state).width
}))
@withHeaderInfoContext()
...
class DynamicHeader extends Component {
    ...
    static getDerivedStateFromProps({
        ...
        if (headerInfo.length > 0 && scrollTop !== prevState.scrollTop) {
            ...
            let newHeaderInfo = find(headerInfo, ({
                position
            }) => {
    ...
    render() {
        ...
        return ( <
            Container >
            <
            Title { ...currentHeaderInfo
            }
            active noAnimate = {!prevHeaderInfo
            }
            key = {
                currentHeaderInfo.label
            }
            isUp = {
            ...
            /> {
                !!prevHeaderInfo && ( <
                    Title { ...prevHeaderInfo
                    }
                    key = {
                        prevHeaderInfo.label
                    }
                    isUp = {
                        scrollDirection === DIRECTION.DOWN
                    }
