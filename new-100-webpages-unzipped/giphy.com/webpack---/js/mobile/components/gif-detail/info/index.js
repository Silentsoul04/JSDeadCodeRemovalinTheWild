...
export default class GIFInfoOverlay extends PureComponent {
    ...
    fetchViewCount = async id => {
        ...
        try {
            const {
                viewCount
            } = await fetchGifDetailViewCount(id)
    ...
    render() {
        ...
        return ( <
            ...
            Footer >
            <
            CopyGIF isShareLauncher = {
                isShareLauncher
            }
            /> {
                isShareLauncher && viewCount > 0 && ( <
                    ViewCount className = "ss-view" > {
                        getShortViewCountDisplay(viewCount)
                    } < /ViewCount>
                )
            } <
            /Footer> <
            ...
            /Tappable> {
                !isShareLauncher && < CopyGifOverlay isCopying = {
                    isCopying
                }
                />} <
                /CopyWrapper> <
