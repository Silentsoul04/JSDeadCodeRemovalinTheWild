...
import {
    setIsMetaVisible,
    setInstagramVisible
} from '../shared/reducers/gif-detail'
import {
    setIsSearchVisible
...
} from 'shared/util/schema-markup'
...
@connect(
    (state, {
        ...
        const gif = isPrivate ? sorryGif : gifs.find(({
            id
        }) => id === idFromPathname)
...
class GifDetailPage extends PureComponent {
    ...
    componentDidUpdate(prevProps) {
        ...
        if (isSearchVisible !== prevProps.isSearchVisible) {
            ...
            this.animationTimeout = setTimeout(() => this.setState({
    ...
    render() {
        ...
        const realSearchBarStyle = hasInteracted && animationComplete && isSearchVisible ? {} : {
            display: 'none'
        }
        const dummySearchBarStyle = !animationComplete && hasInteracted ? {} : {
            display: 'none'
        }
        return ( <
            div className = {
                isOffScreen ? pageCss.contentHidden : pageCss.content
            } >
            <
            Header / >
            <
            SearchBar isOffScreen = {
                isOffScreen
            }
            style = {
                realSearchBarStyle
            ...
            /> <
            DummySearchBarAnimator isSearchVisible = {
                isSearchVisible
            }
            style = {
                dummySearchBarStyle
