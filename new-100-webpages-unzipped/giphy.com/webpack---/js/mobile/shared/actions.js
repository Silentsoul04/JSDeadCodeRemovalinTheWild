...
import {
    resetNewGifs
} from 'shared/redux/new-gifs'
import routes from '../../shared/routes'
import {
    GET,
    POST
} from 'shared/util/fetch-methods'
 ...
 */
export const init = (route, queryString = {}, data = {}, cookies) => ({
    type: INIT,
    route,
    queryString,
    data,
...
export const categoryMenuVisible = visible => ({
    type: CATEGORY_MENU_VISIBLE,
    visible
})

export const USER_MENU_VISIBLE = 'USER_MENU_VISIBLE'
export const userMenuVisible = visible => ({
    type: USER_MENU_VISIBLE,
    visible
})

export const LOGIN_STATUS = 'LOGIN_STATUS'
export const loginStatus = (isFetching, hasChecked) => ({
    type: LOGIN_STATUS,
    isFetching,
    hasChecked
})
...
export const clearQueuedFavorite = () => ({
    type: CLEAR_QUEUED_FAVORITE
})
...
export const setAppSize = (width, height) => ({
    type: APP_SIZE,
    width,
    height
})
    ...
    return function(dispatch, getState) {
        ...
        if (!user.loggedIn) {
            ...
            return (
                ...
                .then(({
                    result
                }) => result.has_favorited)
                // dispatch add or remove which update the state
                .then(isFavorite => {
                    const action = {
                        gifIds: [gifID],
                        groupId: routes.FAVORITES.path
                    }
 ...
 * Settings
 */
export const settings = (data, cookies) => ({
    type: SETTINGS,
    data,
    cookies
})
