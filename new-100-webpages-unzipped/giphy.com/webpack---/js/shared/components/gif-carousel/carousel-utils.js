...
const getStyle = (gif, maxWidth, maxHeight) => {
    const {
        width,
        height
    } = fitGifToSize(gif, maxWidth - MIN_PEEKINESS * 2, maxHeight)
    return {
        width,
        height,
        marginLeft: -width / 2,
...
export const getGifStyles = ({
    ...
    next: nextGif ? getStyle(nextGif, width, height) : {
        width: width / 2
    },
    previous: previousGif ? getStyle(previousGif, width, height) : {
        width: width / 2
    },
})