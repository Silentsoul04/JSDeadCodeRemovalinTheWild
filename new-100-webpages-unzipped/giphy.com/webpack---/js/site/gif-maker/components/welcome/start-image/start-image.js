...
const pickGifs = (gifs, campaignGifs) => {
    ...
    const {
        ...
        id
    }) => (isCanvas(id) ? 'regularGifs' : 'blankCanvas'))
...
class StartImage extends PureComponent {
        ...
        if (isEmpty(gifs)) {
            ...
            canvasGifs.forEach(async ({
                ...
                try {
                    const {
                        results: gifs
                    } = await channelGifs(channelId, 50)
                    receivedGifs({
                        gifs,
                        groupId
                    })
    ...
    componentWillReceiveProps({
        ...
        if (this.props.gifs !== gifs || this.props.campaignGifs !== campaignGifs) {
            this.setState({
                gifs: pickGifs(gifs, campaignGifs)
            })
        }
    }
    ...
    render() {
        ...
        return ( <
            ...
            h3 className = {
                css.title
            } > PICK A CANVAS TO START WITH < /h3> <
            div className = {
                css.description
            } > Add captions, stickers, or draw starting with one of these < /div> {
                !isEmpty(gifs) && [ <
                    Grid key = "grid"
                    gifs = {
                        gifs
                    ...
                    div key = "footer"
                    className = {
                        css.startImageFooter
                    } >
                    <
                    div className = {
                        css.backToWelcomeButton
                    }
                    onClick = {
                        () => replace(GIF_MAKER)
                    } >
                    <
                    span className = {
                        css.startImageBackArrow
                    }
                    /> Back to upload <
                    /div> <
                    div
                    className = {
                        css.startImageShuffleButton
                    }
                    onClick = {
                        () => this.setState({
                            gifs: pickGifs(allGifs)
                        })
                    } >
                    <
                    span className = {
                        css.startImageShuffleIcon
                    }
                    />
                    Shuffle <
                    /div> <
