...
var TrackJS = (function(g, l, n) {
    ...
    t.prototype = {
        watch: function() {
            c.forEach(["EventTarget", "Node", "XMLHttpRequest"], function(a) {
        ...
        wrap: function(a) {
            ...
            var f, e;
            if (d.config.bindStack) try {
                throw Error();
            } catch (m) {
                e = m.stack, f = c.isoNow()
            }
        ...
        wrapEventTarget: function(a) {
            ...
            c.has(a, "addEventListener.call") && c.has(a, "removeEventListener.call") && (c.patch(a, "addEventListener",
                function(a) {
                    return function(f, e, h, m) {
                        try {
                            c.has(e, "handleEvent") && (e.handleEvent = b.wrap(e.handleEvent))
                        } catch (k) {}
                        return a.call(this, f, b.wrap(e), h, m)
                    }
                }), c.patch(a, "removeEventListener", function(a) {
                return function(b, e, h, c) {
                    try {
                        e = e && (e.__trackjs_state__ || e)
                    } catch (k) {}
                    return a.call(this, b, e, h, c)
                }
            }))
        },
            ...
            c.patch(g, a, function(a) {
                return function(f, e) {
                    var h = Array.prototype.slice.call(arguments),
                        m = h[0];
                    c.isFunction(m) && (h[0] = b.wrap(m));
                    return c.has(a, "apply") ? a.apply(this, h) :
                        a(h[0], h[1])
                }
            })
        }
    ...
    p.prototype = {
            ...
            serialize: function(a) {
                ...
                try {
                    d = JSON.stringify(a, function(a, d) {
                        return d === n ? "undefined" : c.isNumber(d) &&
                            isNaN(d) ? "NaN" : c.isError(d) ? {
                                name: d.name,
                                message: d.message,
                                stack: d.stack
                            } : c.isElement(d) ? b(d) : d
        ...
        initCurrent: function(a) {
            if (this.validate(a, this.defaults, "config", {})) return this.current = c.defaultsDeep({}, a, this.defaults), !0;
            this.current = c.defaultsDeep({}, this.defaults);
            console.log("init current config", this.current);
            return !1
        },
        setCurrent: function(a) {
            return this.validate(a, this.defaults, "config", this.initOnly) ? (this.current = c.defaultsDeep({}, a, this.current), !0) : !1
        },
        validate: function(a,
            b, d, f) {
            var e = !0;
            ...
            for (var c in a)
                if (a.hasOwnProperty(c))
                    if (b.hasOwnProperty(c)) {
                        var g = typeof b[c];
                        g !== typeof a[c] ? (console.warn(d + "." + c + ": property must be type " + g + "."), e = !1) : "[object Array]" !== Object.prototype.toString.call(a[c]) || this.validateArray(a[c], b[c], d + "." + c) ? "[object Object]" === Object.prototype.toString.call(a[c]) ? e = this.validate(a[c], b[c], d + "." + c, f[c]) : f.hasOwnProperty(c) && (console.warn(d + "." + c + ": property cannot be set after load."), e = !1) : e = !1
                    } else console.warn(d + "." + c +
                        ": property not supported."), e = !1;
            return e
        },
        validateArray: function(a, b, d) {
            var f = !0;
            d = d || "";
            for (var e = 0; e < a.length; e++) c.contains(b, a[e]) || (console.warn(d + "[" + e + "]: invalid value: " + a[e] + "."), f = !1);
            return f
        }
    };
    var q = function(a, b, d, f, e, c, g) {
        this.util = a;
        this.log = b;
        this.onError = d;
        this.onFault = f;
        this.serialize = e;
        g.enabled && (c.console = this.wrapConsoleObject(c.console, g))
    };
    q.prototype = {
            ...
            for (e = 0; e < b.watch.length; e++)(function(e) {
                ...
                a[e] = function() {
                    try {
                        ...
                        f.log.add("c", {
                            timestamp: f.util.isoNow(),
                            severity: e,
                            message: f.serialize(1 === a.length ? a[0] : a)
                        });
                        if (b[e])
                            if (c.isError(a[0]) && 1 === a.length) f.onError("console", a[0]);
                            else try {
                                throw Error(f.serialize(1 === a.length ? a[0] : a));
                            } catch (d) {
                                f.onError("console", d)
                            }
                        b.display && (f.util.hasFunction(g, "apply") ? g.apply(this, a) : g(a[0]))
                    } catch (d) {
                        f.onFault(d)
                    }
                }
    ...
    v.prototype = {
        discoverDependencies: function() {
            ...
            for (var b in g)
                if ("_trackJs" !== b && "_trackJS" !== b && "_trackjs" !== b && "webkitStorageInfo" !== b && "webkitIndexedDB" !== b &&
                    "top" !== b && "parent" !== b && "frameElement" !== b) try {
                    if (g[b]) {
    ...
    w.prototype = {
        all: function(a) {
            var b = [],
                d, f;
            for (f = 0; f < this.appender.length; f++)(d = this.appender[f]) && d.category === a && b.push(d.value);
            return b
        },
        clear: function() {
        ...
        get: function(a, b) {
            var d, f;
            for (f = 0; f < this.appender.length; f++)
                if (d = this.appender[f], d.category === a && d.key === b) return d.value;
            return !1
        }
    };
    var D = function(a) {
            ...
            return {
                ...
                report: function() {
                    var d = [],
                        f;
                    for (f in b) b.hasOwnProperty(f) && d.push({
                        key: f,
                        value: a(b[f])
                    });
                    return d
    ...
    x.prototype = {
        isCompatible: function(a) {
            a = a || g;
            return !c.has(a, "chrome.app.runtime") &&
                c.has(a, "addEventListener") && c.has(a, "history.pushState")
        },
        ...
        watch: function() {
            if (this.isCompatible()) {
                ...
                c.forEach(["pushState", "replaceState"], function(d) {
                    c.patch(history, d, function(f) {
                        return function() {
                            b =
                                c.getLocationURL().relative;
    ...
    y.prototype = {
        initialize: function(a) {
            ...
            a.XDomainRequest && this.util.hasFunction(a.XDomainRequest.prototype.open, "apply") &&
        ...
        watchFetch: function() {
            ...
            c.patch(g, "fetch", function(f) {
                return function(e, h) {
                    ...
                    var k = e instanceof Request ? e.url : e,
                        C = e instanceof Request ? e.method : (h || {}).method || "GET",
                        l = f.apply(g, arguments);
                    l.__trackjs_state__ = a.add("n", {
                        type: "fetch",
                        startedOn: c.isoNow(),
                        method: C,
                        url: c.truncate(k, 2E3)
                    });
                    return l.then(function(f) {
                        var e =
                            a.get("n", l.__trackjs_state__);
                        if (e) {
                            c.defaults(e, {
                            ...
                            var h = f.headers.get("trackjs-correlation-id");
                            h && (e.requestCorrelationId = h);
                            b.error && 400 <= f.status && (e = Error(e.statusCode + " " + e.statusText + ": " + e.method + " " + e.url), e.stack = m, d("ajax", e))
                        }
                        return f
                    })["catch"](function(f) {
                        f = f || {};
                        var e = a.get("n", l.__trackjs_state__);
                        e && (c.defaults(e, {
                            completedOn: c.isoNow(),
                            statusCode: 0,
                            statusText: f.toString()
                        }), b.error && (f.message = f.message + ": " + e.method +
                            " " + e.url, f.stack = f.stack || m, d("ajax", f)));
                        throw f;
                    })
                }
            })
        ...
        watchNetworkObject: function(a) {
            ...
            a.prototype.open = function(a, b) {
                var f = (b || "").toString();
                0 > f.indexOf("localhost:0") && (this._trackJs = {
                    method: a,
                    url: f
                });
            ...
            a.addEventListener ? a.addEventListener("load", function() {
                ...
                try {
                    ...
                    a.onerror = function() {
                        b.finalizeNetworkEvent(a);
                        b.checkNetworkFault(a);
                        "function" === typeof oldOnError && f.apply(a, arguments)
                    }
                } catch (e) {
                    b.onFault(e)
                }
        ...
        finalizeNetworkEvent: function(a) {
            if (a._trackJs) {
                ...
                b && (b.completedOn = this.util.isoNow(), a.getAllResponseHeaders && a.getResponseHeader && 0 <= (a.getAllResponseHeaders() || "").toLowerCase().indexOf("trackjs-correlation-id") && (b.requestCorrelationId =
                    a.getResponseHeader("trackjs-correlation-id")), b.statusCode = 1223 == a.status ? 204 : a.status, b.statusText = 1223 == a.status ? "No Content" : a.statusText)
            }
        },
    ...
    r.prototype = {
        ...
        sendTrackerFault: function(a) {
            this.throttle(a) || (l.createElement("img").src = this.trackerFaultEndpoint(a))
        },
        ...
        sendError: function(a,
            ...
            if (!this.disabled && !this.throttle(a)) try {
                var f = this.getCORSRequest("POST", this.errorEndpoint(b));
                f.onreadystatechange = function() {
                    4 !== f.readyState || c.contains([200, 202], f.status) || (d.disabled = !0)
                };
                f._trackJs = n;
                f.send(g.JSON.stringify(a))
            } catch (e) {
    ...
    var c = function() {
            function a(d, f, e, h) {
                e = e || !1;
                h = h || 0;
                c.forEach(f, function(f) {
                    c.forEach(c.keys(f), function(c) {
                        null === f[c] || f[c] === n ? d[c] = f[c] : e && 10 > h && "[object Object]" === b(f[c]) ? (d[c] = d[c] || {}, a(d[c], [f[c]], e, h + 1)) : d.hasOwnProperty(c) || (d[c] = f[c])
                    })
                });
                return d
            }
            ...
            return {
                addEventListenerSafe: function(a,
                    b, e, c) {
                    a.addEventListener ? a.addEventListener(b, e, c) : a.attachEvent && a.attachEvent("on" + b, e)
                },
                afterDocumentLoad: function(a) {
                    var b = !1;
                    "complete" === l.readyState ? c.defer(a) : (c.addEventListenerSafe(l, "readystatechange", function() {
                ...
                defaults: function(d) {
                    return a(d, Array.prototype.slice.call(arguments,
                        1), !1)
                },
                ...
                forEach: function(a, b, e) {
                    if (c.isArray(a)) {
                        ...
                        for (var h = 0; h < a.length;) b.call(e, a[h], h, a), h++
                    }
                },
                ...
                has: function(a, b) {
                    try {
                        for (var e = b.split("."), c = a, g = 0; g < e.length; g++)
                            if (c[e[g]]) c = c[e[g]];
                            else return !1;
                        return !0
                    } catch (k) {
                ...
                isError: function(a) {
                    ...
                    var f = b(a);
                    return "[object Error]" === f || "[object DOMException]" === f || c.isString(a.name) && c.isString(a.message)
                },
    ...
    z.prototype = {
        initialize: function(a) {
            ...
            a.addEventListener ? (a.addEventListener("click", b, !0), a.addEventListener("blur", d, !0)) : a.attachEvent &&
        ...
        onSelectInputChanged: function(a) {
            if (a.multiple)
                for (var b = 0; b < a.options.length; b++) a.options[b].selected &&
        ...
        writeVisitorEvent: function(a, b, d, c) {
            ...
            var e = this.getElementAttributes(a);
            a.innerText && (e.__trackjs_element_text = this.util.truncate(a.innerText, 500));
            this.log.add("v", {
        ...
        getElementAttributes: function(a) {
            for (var b = {}, d = Math.min(a.attributes.length, 10), f = 0; f < d; f++) {
                var e = a.attributes[f];
                c.contains(["data-value", "value"], e.name.toLowerCase()) ||
                    (b[e.name] = c.truncate(e.value, 100))
            }
            return b
        },
        ...
        matchInputPattern: function(a) {
            return "" === a ? "empty" : /^[a-z0-9!#$%&'*+=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/.test(a) ? "email" : /^(0?[1-9]|[12][0-9]|3[01])[\/\-](0?[1-9]|1[012])[\/\-]\d{4}$/.test(a) || /^(\d{4}[\/\-](0?[1-9]|1[012])[\/\-]0?[1-9]|[12][0-9]|3[01])$/.test(a) ? "date" :
                /^(?:(?:\+?1\s*(?:[.-]\s*)?)?(?:\(\s*([2-9]1[02-9]|[2-9][02-8]1|[2-9][02-8][02-9])\s*\)|([2-9]1[02-9]|[2-9][02-8]1|[2-9][02-8][02-9]))\s*(?:[.-]\s*)?)?([2-9]1[02-9]|[2-9][02-9]1|[2-9][02-9]{2})\s*(?:[.-]\s*)?([0-9]{4})(?:\s*(?:#|x\.?|ext\.?|extension)\s*(\d+))?$/.test(a) ? "usphone" : /^\s*$/.test(a) ? "whitespace" : /^\d*$/.test(a) ? "numeric" : /^[a-zA-Z]*$/.test(a) ? "alpha" : /^[a-zA-Z0-9]*$/.test(a) ? "alphanumeric" : "characters"
        },
        report: function() {
    ...
    var A = function(a, b, d, c, e) {
        ...
        this.onFault = b;
        this.serialize = d;
        e.enabled && this.watchWindowErrors(c);
        e.promise && this.watchPromiseErrors(c)
    };
    A.prototype = {
        watchPromiseErrors: function(a) {
            ...
            a.addEventListener ? a.addEventListener("unhandledrejection", function(a) {
                a = a || {};
                a = a.detail ? c.tryGet(a.detail, "reason") : c.tryGet(a, "reason");
                if (a !== n) {
                    if (!c.isError(a)) try {
        ...
        watchWindowErrors: function(a) {
            ...
            c.patch(a, "onerror", function(a) {
                return function(c, e, g, m, k) {
                    try {
                        k = k || {}, k.message = k.message || b.serialize(c), k.name = k.name || "Error", k.line = k.line || parseInt(g, 10) || null, k.column = k.column || parseInt(m, 10) || null, "[object Event]" !== Object.prototype.toString.call(c) || e ? k.file = k.file || b.serialize(e) : k.file = (c.target || {}).src, b.onError("window", k)
                    } catch (l) {
                        b.onFault(l)
                    }
    ...
    B.prototype = {
        install: function(a) {
            try {
                if ("undefined" === typeof l) return this.warn("monitoring disabled in node"), !1;
                if (!c.has(a, "token")) return this.warn("missing token"), !1;
                if (this.hasInstalled) return this.warn("already installed"), !1;
                this.config = new p(a);
                this.transmitter = new r(this.util, this.config);
                this.environment = new v(this.config);
                this.customer = new u(this.config, this.util, this.log, this.window, this.document);
                if (!this.config.current.enabled) return this.hasEnabled = !1;
                this.windowConsoleWatcher = new q(this.util, this.log, this.onError, this.onFault, this.serialize, this.window, this.config.current.console);
                if (!this.util.isBrowserSupported()) return !1;
                this.callbackWatcher = new t(this.config.current.callback, this.onError, this.onFault);
                this.visitorWatcher =
                    new z(this.util, this.log, this.onError, this.onFault, this.document, this.config.current.visitor);
                this.navigationWatcher = new x(this.log, this.config.current.navigation);
                this.networkWatcher = new y(this.util, this.log, this.onError, this.onFault, this.window, this.config.current.network);
                this.windowWatcher = new A(this.onError, this.onFault, this.serialize, this.window, this.config.current.window);
                var b = this;
        ...
        pub: function() {
            var a = this,
                b = {
                    ...
                    attempt: function(b, f) {
                        try {
                            var e = Array.prototype.slice.call(arguments, 2);
                            return b.apply(f || this, e)
                        } catch (g) {
                    ...
                    track: function(b) {
                        if (!a.hasInstalled && a.hasEnabled) a.warn("agent must be installed");
                            ...
                            var f = c.isError(b) ? b.message : a.serialize(b);
                            b = b || {};
                            if (!b.stack) try {
                                throw Error(f);
                            } catch (e) {
                    ...
                    watchAll: function(a) {
                        var b = Array.prototype.slice.call(arguments, 1),
                            e;
                        for (e in a) "function" !== typeof a[e] || c.contains(b, e) || (a[e] = this.watch(a[e], a));
                        return a
                    }
                };
            new q(c, a.log, a.onError, a.onFault, a.serialize, b, p.prototype.defaults.console);
            return b
        },
        onError: function() {
            var a, b = !1;
            return function(d, f, e) {
                if (this.hasInstalled && this.hasEnabled && c.isBrowserSupported()) try {
                    if (e = e || {
                            ...
                            name: "Error",
                            message: this.serialize(f, e.force)
                        }), -1 === f.message.indexOf("TrackJS Caught"))
                        if (b &&
                            -1 !== f.message.indexOf("Script error")) b = !1;
                        else {
                            ...
                            if (!e.force) try {
                                if (!this.config.current.onError(h, f)) return
                            } catch (l) {
                                h.console.push({
                                    timestamp: this.util.isoNow(),
                                    severity: "error",
        ...
        serialize: function(a, b) {
            if (this.hasInstalled && this.config.current.serialize && !b) try {
            ...
            return p.prototype.defaults.serialize(a)
        },
        warn: function(a) {
    ...
    return (new B).pub()
})("undefined" === typeof window ? void 0 : window, "undefined" === typeof document ? void 0 : document);
