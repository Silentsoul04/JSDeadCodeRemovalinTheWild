...
(function(a7, K) {
    ...
    b.ipost = function(br, bu, bx, bw) {
        function by(bC, bz) {
            var bB = "";
            for (var bA in bC) {
                if (bC.hasOwnProperty(bA)) {
                    if (typeof bC[bA] == "object") {
                        if (bz) {
                            bB += by(bC[bA], bz + "[" + bA.toString() + "]")
                        } else {
                            bB += by(bC[bA], bA.toString())
                        }
                    } else {
                        if (bz) {
                            if (bC[bA] != K) {
                                bB += '<input type="hidden" name="' + bz + "[" + bA.toString() + ']" value="' + bC[bA].toString() + '" />'
                            }
                        } else {
                            if (bC[bA] != K) {
                                bB += '<input type="hidden" name="' + bA.toString() + '" value="' + bC[bA].toString() + '" />'
                            }
                        }
                    }
                }
        ...
        var bt = b("#ajax_post_ifr");
        if (bt.length == 0) {
            bt = b('<iframe id="ajax_post_ifr" name="ajax_post_ifr" style="display: none;"></iframe>').appendTo("body")
        }
        var bv = '<form action="' + br + '" method="post" target="ajax_post_ifr">';
        if (typeof bu == "object") {
            bv += by(bu)
        } else {
            if (typeof bu == "string") {
                var e = bu.split("&");
                b.each(e, function(bA, bz) {
                    var bB = bz.split("=");
                    if (typeof bB == "object" && bB.length == 2) {
                        bv += '<input type="hidden" name="' + bB[0] + '" value="' + bB[1] + '" />'
                    }
                })
            }
        }
        bv += "</form>";
        var bs = b(bv).appendTo("body");
        bt.unbind("load").load(function() {
            if (typeof bx == "function") {
                var bz = b(this).contents().find("body").html();
                if (bw == "xml") {
                    bz = b.parseXML(bz)
                } else {
                    if (bw == "json") {
