(function() {
    var ccJs = function(options) {
        var mode = "BCP";
        var useFPC = false;
        var includeReferrer = false;
        var runOptimusOnly = false;
        var cmp = {
            daisybit: null,
            wait: 250,
            done: 0,
        ...
        var baseDomain = ".crwdcntrl.net";
        var validPrefix = ["bcp", "cn"];
        var bcpDomainPrefix = "bcp",
            async_props = {
                async: false,
                asyncBehaviors: {},
                asyncOpportunityIds: [],
        ...
        var domain = document.domain,
            ...
            delim = "/",
            autoFire = false;
        var _cc_testCkeName = "_cc_cc";
        var _cc_testCkeVal = "ctst";
        var referrer;
        var _cc_idCkeName = "_cc_id";
        var client, bcpClient;
        client = bcpClient = 12666;
        var firstPartyHost = "cc.theguardian.com";
        var actualClient = {
            id: 12666,
            useFirstPartyDomain: "never",
            domainPrefix: "cc",
            cookieDomain: "theguardian.com",
            cookieDomainIDN: "theguardian.com"
        };
        useFPC = /^(?!.*(chrome|crios).*).*(safari\/\d+|\[fban\/fbios|firefox\/\d+).*/.test(navigator.userAgent.toLowerCase());
        includeReferrer = /^(?!.*chrome.*).*safari\/\d+.*/.test(navigator.userAgent.toLowerCase());
        var useFirstPartyDomain = false;

        function setCMPWait(waitMs) {
            cmp.wait = waitMs
        }
        ...
        function parseQueryStringToParameters(doc) {
            var qs = parseQuery(running.src.replace(/^[^\?]+\??/, ""));
            ns = qs["ns"] || "LOTCC_" + client;
            if (mode == "BCP") {
                autoFire = !!(qs["autobcp"] || false);
                if (typeof qs["pre"] != "undefined") setBCPDomainPrefix(qs["pre"][0]);
                if (typeof qs["cmpwait"] != "undefined") setCMPWait(qs["cmpwait"][0]);
                if ("true" == qs["runOptimusOnly"]) runOptimusOnly =
                    true;
                delete qs["ns"];
                delete qs["autobcp"];
                delete qs["pre"];
                delete qs["cmpwait"];
                delete qs["runOptimusOnly"];
                for (var type in qs)
                    if (isArray(qs[type]))
        ...
        if (running) parseQueryStringToParameters(running);
        ...
        function setBCPDomainPrefix(prefix) {
            if (validPrefix.indexOf(prefix) > 0) bcpDomainPrefix = prefix;
            else if (typeof console != "undefined") console.log(prefix +
                " is not valid")
        }

        function baseURL(is_pv, version, domainArg, sepArg, delimArg) {
            if (typeof version == "undefined" || version == null) version = "5";
            is_pv = is_pv && !namespace.pvdone;
            var domainToUse;
            if (typeof domainArg != "undefined" && domainArg != null) domainToUse = domainArg;
            else domainToUse = getDomain();
            var url = getPrefix() + domainToUse;
            var sepToUse = getDefinedValue(sepArg, sep);
            var delimToUse = getDefinedValue(delimArg, delim);
            url += "/" + version + sepToUse + "c\x3d" + bcpClient + delimToUse + "rand\x3d" + getRandomNumber();
            if (cmp["daisybit"] !=
                null) url += delimToUse + "db\x3d" + cmp["daisybit"];
        ...
        function readData(key) {
            var retVal = getCookie(key);
            if (!retVal && useLocalStorage) retVal = readLocalStorage(key);
            return retVal
        }
        ...
        function getCookie(cookieName) {
            ...
            if (cookieString.length != 0) {
                var cookieMatch = cookieString.match(cookieName + "\x3d([^;]*)");
                if (typeof cookieMatch !=
                    "undefined" && cookieMatch != null) cookieValue = cookieMatch[1]
            }
            return cookieValue
        }
        ...
        function handleCookies(data) {
            ...
            if (firstPartyCookieDomain != null)
                if (data.cookies && data.cookies != "undefined" && data.cookies != null && data.cookies.length > 0) cookieSuccess = writeData(data.cookies, firstPartyCookieDomain);
        ...
        function getFirstPartyCookieDomain(data) {
            var cookieDomain =
                null;
            if (typeof data !== "undefined" && data.domain && data.domain != "undefined" && data.domain != null && actualClient.cookieDomain.length > 0) {
                var domainCandidate = document.domain;
                if (actualClient.cookieDomain == "." + domainCandidate || actualClient.cookieDomain === domainCandidate) cookieDomain = actualClient.cookieDomain;
                else if (actualClient.cookieDomainIDN == "." + domainCandidate || actualClient.cookieDomainIDN === domainCandidate) cookieDomain = actualClient.cookieDomainIDN;
                else if (domainCandidate.indexOf(actualClient.cookieDomain) !==
                    -1) cookieDomain = actualClient.cookieDomain;
                else if (domainCandidate.indexOf(actualClient.cookieDomainIDN) !== -1) cookieDomain = actualClient.cookieDomainIDN
            }
            return cookieDomain
        }
        ...
        function writeData(cookieArray, domainArg) {
            ...
            var localStorageSuccess = writeLocalStorage(cookieArray, domainArg);
        ...
        function writeCookies(cookieArray, domainArg) {
            if (typeof cookieArray != "undefined" && cookieArray != null) {
                ...
                for (var i = 0; i < cookieCount; i++) {
                    ...
                    if (cookie.name == "undefined" || cookie.name == null || cookie.value == "undefined" || cookie.value == null) continue;
                    var cookieString = "";
                    cookieString += cookie.name + "\x3d" + cookie.value + ";";
                    var kvs = new Object;
                    kvs["domain"] = domainArg;
                    kvs["path"] = getCookiePropertyWithDefault(cookie, "path", "/");
                    var maxAge = getCookieProperty(cookie, "maxAge");
                    if (maxAge != -1) {
                        var d = new Date;
                        d.setTime(d.getTime() + maxAge * 1E3);
                        kvs["expires"] = d.toGMTString()
                    }
                    var val;
                    for (var key in kvs) {
                        val =
                            kvs[key];
                        if (typeof val != "undefined" && val != null) cookieString += key + "\x3d" + val + ";"
                    }
        ...
        function writeLocalStorage(cookieArray, domainArg) {
            if (useLocalStorage && typeof cookieArray != "undefined" && cookieArray != null) {
                ...
                for (var i = 0; i < cookieCount; i++) {
                    var cookie = cookieArray[i];
                    if (cookie.name !== _cc_idCkeName || cookie.value == "undefined" || cookie.value == null) continue;
        ...
        function writeTestCookie(returnedDomain) {
            document.cookie = _cc_testCkeName + "\x3d" + _cc_testCkeVal + ";" + "domain\x3d" + returnedDomain + ";";
            var cookieValue = getCookie(_cc_testCkeName);
            if (cookieValue == _cc_testCkeVal) return true;
            else return false
        }

        function getCookieProperty(cookie, property) {
            if (cookie[property] != "undefined" && cookie[property] != null) return cookie[property];
            else return null
        }

        function getCookiePropertyWithDefault(cookie, property, defaultVal) {
            if (cookie[property] != "undefined" && cookie[property] != null) return cookie[property];
            else return defaultVal
        }
        ...
        function getReferrer() {
            if (!referrer) {
                if ("referrer" in options) referrer = options.referrer;
                else if (window.document.referrer !== "") referrer = window.document.referrer;
                else referrer = window.location.href;
                var queryIndex = referrer.indexOf("?");
        ...
        namespace.loaded = "loaded" in options ?
        ...
        for (var prop in async_props)
            if (prop in
                namespace) async_props[prop] = namespace[prop];
        ...
        namespace.setTest =
            setTest;
        namespace.setBCPDomainPrefix = setBCPDomainPrefix;
        namespace.setConsent = setConsent;
        namespace.getConsent = getConsent;
        namespace.getConsentCB = getConsentCB;
        namespace.setConsentCB = setConsentCB;
        LOTCC = namespace;
        addEvent(window, "load", doneload);
        addEvent(window, "load", loadAsync);
        if (autoFire) bcp();

        function getConsentPre() {
            return getPrefix() + "privacy" + baseDomain + "/consent"
        }

        function setConsent(cb, c, co) {
            consentSetCB = cb;
            var t = document.createElement("script");
            t.type = "text/javascript";
            t.src = fpcPidAppend(getConsentPre() +
                "/set?c\x3d" + c + (useFPC ? "\x26fpc\x3dy" : "") + "\x26callback\x3d" + ns + ".setConsentCB\x26rand\x3d" + getRandomNumber());
            if (co.hasOwnProperty("analytics")) t.src += "\x26ca\x3d" + (co.analytics ? 1 : 0);
            if (co.hasOwnProperty("datasharing")) t.src += "\x26cds\x3d" + (co.datasharing ? 1 : 0);
            if (co.hasOwnProperty("targeting")) t.src += "\x26cta\x3d" + (co.targeting ? 1 : 0);
            if (co.hasOwnProperty("crossdevice")) t.src += "\x26ccd\x3d" + (co.crossdevice ? 1 : 0);
            t.id = "_lotconsentset";
            document.body.appendChild(t)
        }

        function fpcPidAppend(url) {
            if (useFPC) {
                var id =
                    readData(_cc_idCkeName);
                if (id != null) url += "\x26pid\x3d" + id
            }
            return url
        }

        function getConsent(cb, c) {
            consentGetCB = cb;
            var t = document.createElement("script");
            t.type = "text/javascript";
            t.src = fpcPidAppend(getConsentPre() + "/get?c\x3d" + c + "\x26callback\x3d" + ns + ".getConsentCB\x26rand\x3d" + getRandomNumber());
            t.id = "_lotconsentget";
            document.body.appendChild(t)
        }

        function getConsentCB(data) {
            var scriptTag = document.getElementById("_lotconsentget");
            document.body.removeChild(scriptTag);
            consentGetCB(data["Profile"])
        }

        function setConsentCB(data) {
            var scriptTag =
                document.getElementById("_lotconsentset");
            document.body.removeChild(scriptTag);
            if (!useFPC || data["cookies"] == "undefined" || data["cookies"] != null && data["cookies"].length == 0 || handleCookies(data)) consentSetCB(data["Profile"]);
            else consentSetCB({
                error: 201
            })
        }
        ...
        function appendAll(url) {
            OptimusRule.applyRBData();
            for (var type in behaviors) url = appendInternal(url, behaviors[type], type);
            for (var pl_type in placementOpps) url = append(url, placementOpps[pl_type], pl_type);
            if (placementOpps.p.length >
                0 || placementOpps.pt.length > 0) url = append(url, "y", "dp");
            if (async_props.async) url = append(url, "y", "async");
            return url
        }

        function getDomain() {
            if (useFirstPartyDomain) return "bcp." + (test ? "test." : "") + firstPartyHost;
            else return test ? "bcp.test.lotame.com" : bcpDomainPrefix + baseDomain
        }

        function bcp() {
            if (runOptimusOnly) return;
            if (typeof __cmp != "undefined") {
                ...
                __cmp("getConsentData", null,
                    function(val, success) {
                        cmp.daisybit = val["consentData"];
                        if (cmp.done == 0) {
                            cmp.done = 1;
                            add("cmpwait", Date.now() - cmp.start);
                            add("cmp", "1");
                            bcpinternal()
                        }
                    })
        ...
        function bcpw() {
            ...
            if (len > 0)
                for (var i = 0; i < len; i += 2)
                    if (i + 1 < len) add(arguments[i], arguments[i +
                        1]);
            bcpd()
        }
        ...
        function getWaitingAddValues() {
            var rtn = 0;
            for (var type in behaviors) {
                var vals = behaviors[type];
                if (typeof vals != "undefined") rtn += vals.length
            }
            return rtn
        }

        function executeOptimus() {
            var startCount = getWaitingAddValues();
            addTrigger({
                "int": ["Site : $1"],
                location: "\\://([^/]*)",
                element: "{{html}}",
            ...
            addTrigger({
                "int": ["$1 : Site Section : $2 : $3 : $4 : $5 : $6"],
                location: "\\://([^/]*)/([a-zA-Z0-9_+%\u0080-\uffff-]+)/([a-zA-Z0-9_+%\u0080-\uffff-]+)/([a-zA-Z0-9_+%\u0080-\uffff-]+)/([a-zA-Z0-9_+%\u0080-\uffff-]+)/([a-zA-Z0-9_+%\u0080-\uffff-]+)",
                element: "{{html}}",
                event: "",
                opr: 85001
            });
            addTrigger({
                "med": ["Topic Keywords : {{div.submeta__keywords  ul  li:nth-child(1)  a}}.textContent"],
                location: "([a-zA-Z0-9_-]+)\\.(com)",
                element: "{{div.submeta__keywords  ul  li:nth-child(1)  a}}",
                event: "",
                opr: 85014
            });
            addTrigger({
                "med": ["Topic Keywords : {{div.submeta__keywords  ul  li:nth-child(1)  a}}.textContent,{{div.submeta__keywords  ul  li:nth-child(2)  a}}.textContent"],
                location: "([a-zA-Z0-9_-]+)\\.(com)",
                element: "{{div.submeta__keywords  ul  li:nth-child(2)  a}}",
                event: "",
                opr: 85015
            });
            addTrigger({
                "med": ["Topic Keywords : {{div.submeta__keywords  ul  li:nth-child(1)  a}}.textContent,{{div.submeta__keywords  ul  li:nth-child(2)  a}}.textContent,{{div.submeta__keywords  ul  li:nth-child(3)  a}}.textContent"],
                location: "([a-zA-Z0-9_-]+)\\.(com)",
                element: "{{div.submeta__keywords  ul  li:nth-child(3)  a}}",
                event: "",
                opr: 85016
            });
            addTrigger({
                "med": ["Topic Keywords : {{div.submeta__keywords  ul  li:nth-child(1)  a}}.textContent,{{div.submeta__keywords  ul  li:nth-child(2)  a}}.textContent,{{div.submeta__keywords  ul  li:nth-child(3)  a}}.textContent,{{div.submeta__keywords  ul  li:nth-child(4)  a}}.textContent"],
                location: "([a-zA-Z0-9_-]+)\\.(com)",
                element: "{{div.submeta__keywords  ul  li:nth-child(4)  a}}",
                event: "",
                opr: 85017
            });
            addTrigger({
                "med": ["Topic Keywords : {{div.submeta__keywords  ul  li:nth-child(1)  a}}.textContent,{{div.submeta__keywords  ul  li:nth-child(2)  a}}.textContent,{{div.submeta__keywords  ul  li:nth-child(3)  a}}.textContent,{{div.submeta__keywords  ul  li:nth-child(4)  a}}.textContent,{{div.submeta__keywords  ul  li:nth-child(5)  a}}.textContent"],
                location: "([a-zA-Z0-9_-]+)\\.(com)",
                element: "{{div.submeta__keywords  ul  li:nth-child(5)  a}}",
                event: "",
                opr: 85018
            });
            addTrigger({
                "med": ["Breadcrumbs : {{#tf_breadcrumb a:nth-child(1)}}.textContent"],
                location: "\\://([^/]*)/([a-zA-Z0-9_+%\u0080-\uffff-]+)",
                element: "{{#tf_breadcrumb a:nth-child(1)}}",
                event: "",
                opr: 85027
            });
            addTrigger({
            ...
            addTrigger({
                rbk: "post_title",
                template: ["{{div.content__header h1.content__headline}}.textContent"],
                location: "\\://([^/]*)",
                element: "{{div.content__header h1.content__headline}}",
                event: "",
                opr: 86059
            });
            addTrigger({
                rbk: "post_title",
                template: ["{{section.article__headline-section h1.article__headline}}.textContent"],
                location: "\\://([^/]*)",
                element: "{{section.article__headline-section h1.article__headline}}",
                event: "",
                opr: 86060
            });
            addTrigger({
                rbk: "post_title",
                template: ["{{div.headline h1.headline__heading}}.textContent"],
                location: "\\://([^/]*)",
                element: "{{div.headline h1.headline__heading}}",
                event: "",
                opr: 86061
            });
            addTrigger({
                rbk: "post_title",
                template: ["{{h1.section-theme-background-indicator}}.textContent"],
                location: "\\://([^/]*)",
                element: "{{h1.section-theme-background-indicator}}",
                event: "",
                opr: 86062
            });
            addTrigger({
                rbk: "post_title",
                template: ["{{header.Article-header h1.Article-headline}}.textContent"],
                location: "\\://([^/]*)",
                element: "{{header.Article-header h1.Article-headline}}",
                event: "",
                opr: 86063
            });
            bindTriggers();
            return getWaitingAddValues() - startCount
        }
        ...
        function firebcp() {
            ...
            if (namespace.dropSyncPixels)
                if (useFPC) {
                    var url = generateFirebcpURL("j");
                    makeJsonpCall(url, "jsonpCallback")
                } else {
                    var url = generateFirebcpURL("5");
                    try {
                        var tempIFrame = document.createElement("iframe");
                        tempIFrame.setAttribute("id", "LOTCCFrame_" + (new Date).getTime());
                        url = append(url, "ifr", "rt");
                        tempIFrame.setAttribute("src", url.replace(/'/g, "%27"));
                        tempIFrame.setAttribute("title", "empty");
        ...
        function generateFirebcpURL(version) {
            var url =
                appendAll(baseURL(true, version));
            ...
            if (typeof lotmed != "undefined") url = append(url, lotmed, "med");
            if (includeReferrer) url = append(url, encodeURIComponent(getReferrer()), "ref");
            return url
        }
        ...
        function loadAsync() {
            if (async_props.async) try {
                for (type in async_props.asyncBehaviors)
                    for (var i =
                            0, b; b = async_props.asyncBehaviors[type][i++];) add(type, b);
                for (var i = 0, id; id = async_props.asyncOpportunityIds[i++];) addOpportunityId(async_props.asyncOpportunityIds[id]);
                for (var i = 0, tag; tag = async_props.asyncOpportunities[i++];) addOpportunity(async_props.asyncOpportunities[tag]);
                bcp()
            } catch (e) {}
        }
        var placement_rules = [],
        ...
        function bindTriggers() {
            placement_rules.sort(function(a, b) {
                ...
                if (!regex.test(a.element) &&
                    regex.test(b.element)) return 1;
                return 0
            });
            ...
            if (OptimusRule.element_cache.length > 0)
                for (var i = OptimusRule.element_cache.length - 1; i >= 0; i--) {
                    ...
                    for (var ev in handlers) addEvent(el, ev, function(h) {
                        var evt_el = el;
                        return function(evt) {
                            for (var j = 0, callback; callback = h[j++];) callback.call(evt_el, evt);
                            ...
                            if (hasBehaviors ||
                                OptimusRule.rbData !== null) {
                                if (runOptimusOnly) {
                                    parseQueryStringToParameters(running);
                                    add("amskip", "Y")
                                }
                                bcpw()
                            }
        ...
        OptimusRule.prototype = {
                ...
                for (var href in this.document_map) {
                    ...
                    if (this.element !== null) {
                        ...
                        for (var i = 0, el; el = elm_match[i++];)
                            if (typeof el["property_name"] !== "undefined" && el["property_name"] !==
                                null && el["property_content"] === null) missing_property = true;
            ...
            matchRBK: function(matches) {
                var rbVals = [];
                var locs = this.location_matches;
                var subRegexes = [];
                if (locs !== null && locs.length > 0)
                    for (var i = 0; i < locs.length; i++) subRegexes.push(new RegExp("\\$" +
                        i.toString() + "\\b"));
                var addToVals = function(v) {
                    if (v !== "") {
                        for (var i = 0; i < subRegexes.length; i++) v = v.replace(subRegexes[i], locs[i]);
                        rbVals.push(v)
                    }
                };
                if (matches.length > 0)
                    for (var i = 0, m; m = matches[i++];)
                        if (typeof m.properties !== "undefined" && m.properties.length > 0)
                            for (var x = 0, p; p = m.properties[x++];) addToVals(("" + this.behavior).replace(m.expression, p));
                        else addToVals(("" + this.behavior).replace(m.expression, m.property_content !== null ? m.property_content : ""));
                else {
                    var v = ("" + this.behavior).replace(this.query_regex,
                        "");
                    addToVals(v)
                }
                if (rbVals.length > 0) OptimusRule.addRBData(this.rbk, rbVals)
            },
            add: function(b) {
            ...
            create_event: function(doc) {
                ...
                for (var i = 0, curr_elm; curr_elm = bind_to[i++];) {
                    if (!(curr_elm.nodeType &&
                            (curr_elm.nodeType == 1 || curr_elm.nodeType == 9))) continue;
                    var found = false;
            ...
            query: function(q, doc) {
                ...
                try {
                    ...
                    while ((results = this.query_regex.exec(q)) !== null) {
                        ...
                        els = typeof OptimusRule.supported_sources[prefix] === "function" ? OptimusRule.supported_sources[prefix].call(this, selector,
                            property, doc) : [];
        ...
        OptimusRule.supported_sources = {
            ...
            r: function(s, p, d) {
                ...
                return r_prop !== null ? [matches, r_prop, [r_prop]] : []
            }
        };
        OptimusRule.rbData = null;
        OptimusRule.addRBData = function(k, a) {
            OptimusRule.rbData = OptimusRule.rbData || {};
            if (typeof OptimusRule.rbData[k] === "undefined") OptimusRule.rbData[k] = a.length > 1 ? a : a[0];
            else {
                if (window.toString.call(OptimusRule.rbData[k]) !== "[object Array]") OptimusRule.rbData[k] = [OptimusRule.rbData[k]];
                Array.prototype.push.apply(OptimusRule.rbData[k], a)
            }
        };
        OptimusRule.applyRBData = function() {
            if (OptimusRule.rbData !== null) {
                add("rb", JSON.stringify(OptimusRule.rbData));
                OptimusRule.rbData = null
            }
        };
        ...
        HapaxRule.prototype.is_valid = function() {
            ...
            if (this.element === "" || this.element ===
                null || typeof this.element !== "string") valid = false;
        ...
        PlacementRule.prototype.process = function() {
            if (!this.is_valid) return false;
            var els = this.query(this.query_regex.test(this.element) ?
        ...
        PlacementRule.prototype.is_valid = function() {
            var valid = true;
            if ((this.placement == null || typeof this.placement !== "string") &&
                (this.placement_id == null || isNaN(parseInt(this.placement_id, 10)))) valid = false;
        ...
        (function() {
            ...
            var arrayIt = function(a) {
                if (!!(window.attachEvent &&
                        !window.opera)) return function(a) {
            ...
            var peppy = {
                query: function(selectorGroups, root, includeRoot, recursed, flat) {
                    ...
                    var groupsWorker, groups, selector;
                ...
                querySelector: function(selector, root, includeRoot, flat) {
                    ...
                    while ((aTest = tests.pop()) !== undefined) {
                        var pc = persistCache[aTest],
                            testFuncScope, testFunc, testFuncKey,
                            testFuncArgs = [],
                            isCountTest = false;
                        passed = [];
                        if (pc) {
            ...
            if (doc.querySelectorAll)(function() {
                ...
                if (doc.addEventListener) {
                    doc.addEventListener("DOMAttrModified",
                        clearCache, false);
                    doc.addEventListener("DOMNodeInserted", clearCache, false);
                    doc.addEventListener("DOMNodeRemoved", clearCache, false)
                } else if (doc.attachEvent) {
                    doc.attachEvent("DOMAttrModified", clearCache);
                    doc.attachEvent("DOMNodeInserted", clearCache);
                    doc.attachEvent("DOMNodeRemoved", clearCache)
                }
            }
        ...
        bcp();
        window[ns].jsonpCallback = jsonpCallback;
        var _cc_dcCkeName = "_cc_dc";
        var _cc_cloCkeName = "_cc_clo";

        function makeJsonpCall(url, callbackMethod, delimArg) {
            var delimToUse = getDefinedValue(delimArg, delim);
            var callbackId = "cc_" + callbackMethod;
            var urlString = String(url);
            urlString = append(urlString, ns + "." + callbackMethod, "callback", delimToUse);
            urlString = append(urlString, callbackId, "callbackid", delimToUse);
            urlString = append(urlString, encodeURIComponent(delimToUse),
                "delim", delimToUse);
            var fpcJsonArray = getFirstPartyCookiesJson();
            if (fpcJsonArray.length > 0) {
                var jsonPayload = [createJsonPayload(fpcJsonArray)];
                urlString = append(urlString, jsonPayload, "jsonpayload", delimToUse);
                if (urlString.indexOf("fpc\x3dy") == -1) urlString = append(urlString, "y", "fpc", delimToUse)
            }
            var tempScriptTag = document.createElement("script");
            tempScriptTag.type = "text/javascript";
            tempScriptTag.src = urlString;
            tempScriptTag.id = callbackId;
            document.body.appendChild(tempScriptTag)
        }

        function jsonpCallback(data) {
            if (data ==
                null || data == "undefined") return;
            removeJsonpTag(data);
            handleCookies(data);
            var delimToUse = getDefinedValue(data["delim"], delim);
            if (data["redirect"]) {
                var redirectUrl = String(data["redirect"]);
                var redirectType = getDefinedValue(data["redirectType"], "JSONP");
                switch (redirectType) {
                    ...
                    case "JSONP":
                    default:
                        redirectUrl = removeCallbackParams(redirectUrl, data, delimToUse);
                        var firstPartyCookieDomain = getFirstPartyCookieDomain(data);
                        if (redirectUrl.indexOf("ct\x3dy") !=
                            -1) {
                            var isCookieable = false;
                            if (firstPartyCookieDomain != null) isCookieable = writeTestCookie(firstPartyCookieDomain);
                            if (isCookieable) redirectUrl = addFpcParam(redirectUrl)
                        }
                        var url = getPrefix() + getDomain() + redirectUrl;
                        makeJsonpCall(url, "jsonpCallback", delimToUse);
                        break
                }
            }
        }

        function removeJsonpTag(data) {
            if (!data["callbackId"] || data["callbackId"] == "undefined" || data["callbackId"] == null) return;
            var scriptTag = document.getElementById(data["callbackId"]);
            document.body.removeChild(scriptTag)
        }

        function removeCallbackParams(url,
            data, delimArg) {
            var delimToUse = getDefinedValue(delimArg, delim);
            if (typeof data["calback"] !== "undefined" && data["callback"] != "undefined" && data["callback"] != null) url = removeUrlParam(url, delimToUse + "callback\x3d" + data["callback"]);
            if (data["callbackId"]) url = removeUrlParam(url, delimToUse + "callbackid\x3d" + data["callbackId"]);
            if (data["delim"]) url = removeUrlParam(url, delimToUse + "delim\x3d" + data["delim"]);
            return url
        }

        function removeUrlParam(url, param) {
            var array = url.split(param);
            return String(array[0] + array[1])
        }

        function getFirstPartyCookiesJson() {
            var retVal = new Array;
            var fpcId = readData(_cc_idCkeName);
            if (fpcId != null) retVal.push(createCookieJson(_cc_idCkeName, fpcId));
            var fpcDc = getCookie(_cc_dcCkeName);
            if (fpcDc != null) retVal.push(createCookieJson(_cc_dcCkeName, fpcDc));
            var fpcClo = getCookie(_cc_cloCkeName);
            if (fpcClo != null) retVal.push(createCookieJson(_cc_cloCkeName, fpcClo));
            return retVal
        }

        function addFpcParam(url) {
            if (url.indexOf("fpc\x3dy") == -1) url = append(url, "y", "fpc");
            return url
        }

        function createCookieJson(name,
            value) {
            return '{"name":"' + name + '","value":"' + value + '"}'
        }

        function createJsonPayload(cookies) {
            var jsonPayload = "{";
            if (cookies != null && typeof cookies == "object" && cookies.length > 0) {
                var cookiesJson = '"cookies":[';
                var cookiesCount = cookies.length;
                for (var i = 0; i < cookiesCount; i++) cookiesJson += cookies[i] + ",";
                cookiesJson = cookiesJson.substring(0, cookiesJson.length - 1);
                cookiesJson += "]";
                jsonPayload += cookiesJson
            }
            jsonPayload += "}";
            jsonPayload = encodeURI(jsonPayload);
            return jsonPayload
        }
    };
    ccJs()
