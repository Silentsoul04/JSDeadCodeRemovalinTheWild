...
class CartContent extends NamespaceConsumer {
    ...
    handleAddEvent() {
        this.setState({
            placeholders: this.state.placeholders + 1
        });
    ...
    handleDelete(item) {
        ...
        services.basket
            .delete(item)
            .then(() => document.dispatchEvent(new CustomEvent(Events.REMOVE_ITEM, {
                detail: item
            })));
    }
    ...
    handleDurationChange(item, newDuration) {
        const {
            pfid
        } = item.periodOptions.find(option => option.period === newDuration);
        if (item.pfid !== pfid) {
            this.setState({
                editing: true
            });
            services.basket.patch({
                items: [{
                    itemId: item.itemId,
                    pfid
                }]
    ...
    render() {
            ...
            return ( <
                ...
                /div> <
                hr className = {
                    this.namespace(this.classNames('m-b-1 hc-divider', {
                ...
                <
                div className = {
                    this.namespace('hc-items')
                } >
                <
                Expander onOverflow = {
                    this.handleOverflow
                } > {
                    Array(placeholders).fill(0).map((_, index) => < Placeholder key = {
                            index
                        }
                        />)} {
                            reversedItems && reversedItems.map(item =>
                                ...
                                onDurationChange = {
                                    (e) => this.handleDurationChange(item, e.value)
                                }
                                />
                            )
                        } <
