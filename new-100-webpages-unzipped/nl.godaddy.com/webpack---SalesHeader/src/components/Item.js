...
class Item extends NamespaceConsumer {
    render() {
        ...
        return ( <
            div className = {
                this.namespace('hc-item item card ux-card')
            } >
            <
            div className = {
                this.namespace('hc-item-header bg-faint')
            } >
            <
            div className = {
                this.namespace('hc-item-header-text')
            } >
            <
            div className = {
                this.namespace('ms-1')
            ...
            <
            div className = {
                this.namespace('ms-1 text-muted')
            } > {
                subtitle
            } < /div> <
            /div> <
            ...
            <
            span className = {
                this.namespace('uxicon uxicon-trash align-top ms-1')
            }
            /> <
            /Button> <
            /div> <
            div className = {
                this.namespace('hc-item-editor')
            } >
            <
            div > {!isOneTimeFee && < div className = {
                    this.namespace('ms-1')
                } > < strong > < FormattedMessage id = 'Shared:UtilityBar:HeaderCart:purchaseLength' / > < /strong></div >
            } <
            div className = {
                this.namespace('ms-2 text-muted')
            } >
            <
            MultiYearPrice design = 'inline'
            item = {
                item
            }
            currency = {
            ...
            span className = {
                this.namespace('text-success')
            } >
            <
            strong > {
                formatPrice(market, currency, item.pricing.subtotal)
            } < /strong> <
            /span> <
            /div> <
            /div> <
