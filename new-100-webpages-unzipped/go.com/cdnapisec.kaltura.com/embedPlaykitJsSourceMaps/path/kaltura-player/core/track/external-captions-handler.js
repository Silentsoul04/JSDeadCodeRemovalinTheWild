...
class ExternalCaptionsHandler extends FakeEventTarget {
    ...
    hideTextTrack(): void {
        ...
        if (this._isTextTrackActive) {
            ...
            this.dispatchEvent(new FakeEvent(CustomEventType.TEXT_CUE_CHANGED, {
                cues: []
            }));
    ...
    getExternalTracks(tracks: Array < Track > ): Array < TextTrack > {
        ...
        captions.forEach(caption => {
            if (!caption.language) {
                const error = new Error(Error.Severity.RECOVERABLE, Error.Category.TEXT, Error.Code.UNKNOWN_LANGUAGE, {
                    caption: caption
                });
    ...
    _getCuesString(textTrack: TextTrack): Promise < * > {
        return new Promise((resolve, reject) => {
            ...
            if (![SRT_POSTFIX, VTT_POSTFIX].includes(captionType)) {
                ...
                reject(new Error(Error.Severity.RECOVERABLE, Error.Category.TEXT, Error.Code.UNKNOWN_FILE_TYPE, {
...
// WEBPACK FOOTER //
// core/track/external-captions-handler.js