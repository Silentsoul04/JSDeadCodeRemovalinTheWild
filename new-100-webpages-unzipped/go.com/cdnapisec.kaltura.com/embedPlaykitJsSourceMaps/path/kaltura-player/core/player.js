...
import {
    processCues
} from './track/text-track-display';
import {
    StateType
} from './state/state-type';
import {
    TrackType
} from './track/track-type';
import {
    LabelToTrackMap
} from './track/label-to-track-map';
import {
    StreamType
} from './engines/stream-type';
import {
    EngineType
} from './engines/engine-type';
import {
    MediaType
} from './media-type';
import {
    AbrMode
} from './track/abr-mode-type';
import {
    CorsType
} from './engines/html5/cors-types';
...
import {
    EngineProvider
} from './engines/engine-provider';
import {
    ExternalCaptionsHandler
} from './track/external-captions-handler';
import {
    AdBreakType
} from './ads/ad-break-type';
import {
    AdTagType
} from './ads/ad-tag-type';
import {
    AdsController
} from './ads/ads-controller';
import {
    AdEventType
} from './ads/ad-event-type';
import {
    ControllerProvider
} from './controller/controller-provider';
import {
    ResizeWatcher
} from './utils/resize-watcher';
import {
    FullscreenController
} from './fullscreen/fullscreen-controller';
...
export default class Player extends FakeEventTarget {
    ...
    configure(config: Object = {}): void {
        ...
        if (this._hasSources(config.sources)) {
            ...
            if (this._selectEngineByPriority()) {
                this.dispatchEvent(new FakeEvent(CustomEventType.SOURCE_SELECTED, {
        ...
        if (this._engine && !this.src && !this._loading) {
            ...
            this._engine
                ...
                .then(data => {
                    ...
                    this.dispatchEvent(new FakeEvent(CustomEventType.TRACKS_CHANGED, {
                        tracks: this._tracks
                    }));
                    resetFlags();
                })
    ...
    set muted(mute: boolean): void {
        if (this._engine) {
            ...
            this.dispatchEvent(new FakeEvent(CustomEventType.MUTE_CHANGE, {
                mute: mute
            }));
            if (mute === false) {
