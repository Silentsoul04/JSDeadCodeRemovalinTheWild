...
class PlaylistManager {
    ...
    configure(config: ? KPPlaylistObject, entryList : ? ProviderEntryListObject) {
        if (config) {
            ...
            if (config.items && config.items.find(item => !!item.sources)) {
                ...
                this._player.dispatchEvent(new FakeEvent(PlaylistEventType.PLAYLIST_LOADED, {
                    playlist: this
                }));
                this._addBindings();
                this.playNext();
    ...
    get countdown(): KPPlaylistCountdownOptions {
        if (this._playlist.current.item && this._playlist.current.item.config) {
            const mergedConfig: KPPlaylistCountdownOptions = {
                duration: 10,
                showing: true
            };
    ...
    _setItem(activeItem: PlaylistItem, index: number): Promise < * > {
        ...
        if (activeItem.isPlayable()) {
            ...
            this._player.dispatchEvent(new FakeEvent(PlaylistEventType.PLAYLIST_ITEM_CHANGED, {
                index,
                activeItem
            }));
...
// WEBPACK FOOTER //
// ./common/playlist/playlist-manager.js