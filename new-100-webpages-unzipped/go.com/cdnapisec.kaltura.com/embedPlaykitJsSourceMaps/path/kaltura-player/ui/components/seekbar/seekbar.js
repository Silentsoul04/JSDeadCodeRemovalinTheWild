...
import {
    KEYBOARD_DEFAULT_SEEK_JUMP
} from '../keyboard/keyboard';
import {
    bindMethod
} from '../../utils/bind-method';
...
class SeekBarControl extends Component {
    ...
    renderFramePreview(): React$Element < any > | void {
        if (!this.props.config.thumbsSprite ||
        ...
        return ( <
            ...
            style = {
                this._getFramePreviewStyle()
            }
            ref = {
                c => (this._framePreviewElement = c)
            } >
            <
            div className = {
                style.framePreviewImg
            }
            style = {
                this._getFramePreviewImgStyle()
            }
            /> <
            /div>
        );
    ...
    render(props: any): React$Element < any > {
        ...
        return ( <
            div tabIndex = "0"
            className = {
                seekbarStyleClass.join(' ')
            }
            ref = {
                c => (this._seekBarElement = c)
            }
            role = "slider"
            ...
            aria - valuetext = {
                `${toHHMMSS(this.props.currentTime)} of ${toHHMMSS(this.props.duration)}`
            }
            onClick = {
                e => this.onTap(e)
            }
            onMouseOver = {
                () => this.onSeekbarMouseOver()
            }
            onMouseLeave = {
                () => this.onSeekbarMouseLeave()
            }
            onMouseMove = {
                e => this.onSeekbarMouseMove(e)
            }
            onMouseDown = {
                e => this.onSeekbarMouseDown(e)
            }
            onTouchStart = {
                e => this.onSeekbarTouchStart(e)
            }
            onTouchMove = {
                e => this.onSeekbarTouchMove(e)
            }
            onTouchEnd = {
                () => this.onSeekbarTouchEnd()
            }
            onKeyDown = {
                e => this.onSeekbarKeyDown(e)
            } >
            <
            div className = {
                style.progressBar
            ...
            div className = {
                style.virtualProgress
            }
            style = {
                {
                    width: virtualProgressWidth
                }
            }
            /> <
            div className = {
            ...
            style = {
                ...
                props.adBreak ? undefined : < a className = {
                    style.scrubber
                }
                />} <
                /div> <
    ...
    // WEBPACK FOOTER //
    // ui/components/seekbar/seekbar.js