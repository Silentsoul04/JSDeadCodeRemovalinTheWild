...
import {
    actions
} from '../../reducers/volume';
import {
    actions as engineActions
...
import {
    KEYBOARD_DEFAULT_VOLUME_JUMP
} from '../keyboard/keyboard';
import {
    FakeEvent
} from '../../event/fake-event';
...
class VolumeControl extends BaseComponent {
    ...
    render(): React$Element < any > {
        ...
        return ( <
            div ref = {
                c => (this._volumeControlElement = c)
            }
            className = {
                controlButtonClass.join(' ')
            }
            onMouseOver = {
                () => this.onVolumeMouseOver()
            }
            onMouseOut = {
                () => this.onVolumeMouseOut()
            } >
            <
            button tabIndex = "0"
            aria - label = "Volume"
            ...
            onClick = {
                () => this.onVolumeControlButtonClick()
            }
            onKeyDown = {
                e => this.onVolumeControlKeyDown(e)
            } >
            <
            Icon type = {
                IconType.VolumeBase
            ...
            aria - valuetext = {
                `${this.player.volume * 100}% volume ${this.player.muted ? 'muted' : ''}`
            } >
            <
            div className = {
                style.bar
            ...
            ref = {
                c => (this._volumeProgressBarElement = c)
            }
            onMouseDown = {
                () => this.onVolumeProgressBarMouseDown()
            } >
            <
            div className = {
                style.progress
            ...
            style = {
                {
                    height: this.getVolumeProgressHeight()
                }
            }
            /> <
            /div> <
...
// WEBPACK FOOTER //
// ui/components/volume/volume.js