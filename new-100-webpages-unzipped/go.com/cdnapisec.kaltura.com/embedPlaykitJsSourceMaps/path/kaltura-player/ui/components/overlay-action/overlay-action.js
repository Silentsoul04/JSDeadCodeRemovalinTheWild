...
class OverlayAction extends BaseComponent {
    ...
    render(): React$Element < any > {
        return ( <
            div className = {
                `${style.overlayAction} ${this.state.animation ? style.in : ''}`
            }
            onMouseDown = {
                e => this.onOverlayPointerDown(e)
            }
            onTouchStart = {
                e => this.onOverlayPointerDown(e)
            }
            onMouseUp = {
                e => this.onOverlayMouseUp(e)
            }
            onTouchEnd = {
                e => this.onOverlayTouchEnd(e)
            } > {
    ...
    renderIcons(): React$Element < any > {
        if (Array.isArray(this.state.iconType)) {
            return this.state.iconType.map((i, x) => < Icon key = {
                    x
                }
                type = {
                    i
    ...
    // WEBPACK FOOTER //
    // ./components/overlay-action/overlay-action.js


    // WEBPACK FOOTER //
    // ui/components/overlay-action/overlay-action.js