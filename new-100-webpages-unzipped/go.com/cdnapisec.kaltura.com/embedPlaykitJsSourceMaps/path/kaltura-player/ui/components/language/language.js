...
import {
    SmartContainerItem
} from '../smart-container/smart-container-item';
import {
    default as Icon,
    IconType
} from '../icon';
...
class LanguageControl extends BaseComponent {
    ...
    onControlButtonClick(): void {
        this.setState({
            smartContainerOpen: !this.state.smartContainerOpen
        });
    }
    ...
    toggleCVAAOverlay(): void {
        this.setState({
            cvaaOverlay: !this.state.cvaaOverlay
        });
    }
    ...
    renderAll(audioOptions: Array < Object > , textOptions: Array < Object > ): React$Element < any > {
        ...
        return ( <
            div ref = {
                c => (this._controlLanguageElement = c)
            }
            className = {
                [style.controlButtonContainer, style.controlLanguage].join(' ')
            } >
            <
            Localizer >
            <
            ...
            className = {
                this.state.smartContainerOpen ? [style.controlButton, style.active].join(' ') : style.controlButton
            }
            onClick = {
            ...
            /Localizer> {
                !this.state.smartContainerOpen || this.state.cvaaOverlay ? (
                    ...
                    SmartContainer targetId = {
                        this.player.config.targetId
                    }
                    title = "Language"
                    onClose = {
                        () => this.onControlButtonClick()
                        ...
                        audioOptions.length <= 1 ? (
                            ...
                            onSelect = {
                                audioTrack => this.onAudioChange(audioTrack)
                            }
                            /> <
                            /Localizer>
                        )
                        ...
                        textOptions.length <= 1 ? (
                            ...
                            Localizer >
                            <
                            SmartContainerItem icon = "captions"
                            ...
                            onSelect = {
                                textTrack => this.onCaptionsChange(textTrack)
                            }
                            /> <
                            /Localizer>
                        )
                        ...
                        textOptions.length <= 1 ? (
                            ...
                            <
                            a className = {
                                style.advancedCaptionsMenuLink
                            }
                            onClick = {
                                () => this.toggleCVAAOverlay()
                            } >
    ...
    render(props: any): React$Element < any > | void {
        const audioOptions = props.audioTracks.filter(t => t.label || t.language).map(t => ({
...
// WEBPACK FOOTER //
// ui/components/language/language.js