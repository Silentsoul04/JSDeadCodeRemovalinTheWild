...
const ShareButton = (props: Object): React$Element < any > => {
    ...
    return ( <
        ...
        className = {
            [style.btnRounded, style[props.config.iconType], props.config.iconType].join(' ')
        }
        onClick = {
            () => share()
        } >
        <
        Icon style = {
            props.config.iconType === 'svg' ? `background-image: url(${props.config.svg})` : ``
        }
        type = {
...
const ShareUrl = (props: Object): React$Element < any > => {
        ...
        return ( <
            div className = {
                props.copy ? style.copyUrlRow : ''
            } >
            <
            div className = {
                [style.formGroup, style.hasIcon, style.inputCopyUrl].join(' ')
            }
            style = "width: 350px;" >
            <
            input type = "text"
            ...
            /> <
            /div> {
                props.copy && < CopyButton copy = {
                    () => copyUrl(_ref, props.isIos)
                }
                />} <
                /div>
            );
        ...
        const VideoStartOptions = (props: Object): React$Element < any > => {
            return ( <
                div className = {
                    style.videoStartOptionsRow
                } >
                <
                div className = {
                    [style.checkbox, style.dInlineBlock].join(' ')
                } >
                <
                input type = "checkbox"
                id = "start-from"
                ...
                <
                Text id = {
                    'share.start_video_at'
                }
                /> <
                /label> <
                /div> <
                div className = {
                    [style.formGroup, style.dInlineBlock].join(' ')
                } >
                <
                input type = "text"
                className = {
                ...
                onChange = {
                    e => props.handleStartFromChange(e)
                }
                value = {
                    toHHMMSS(props.startFromValue)
                }
                style = "width: 72px;" /
                >
                <
        ...
        class ShareOverlay extends BaseComponent {
            ...
            renderMainState(): React$Element < any > {
                return ( <
                    div className = {
                        this.state.view === shareOverlayView.Main ? 'overlay-screen active' : 'overlay-screen'
                    } >
                    <
                    div className = {
                        style.title
                    ...
                    <
                    a className = {
                        [style.btnRounded, style.emailShareBtn].join(' ')
                    }
                    href = {
                    ...
                    <
                    button className = {
                        [style.btnRounded, style.embedShareBtn].join(' ')
                    }
                    onClick = {
                    ...
                    /> {
                        this.props.enableTimeOffset ? ( <
                            VideoStartOptions startFrom = {
                                this.state.startFrom
                            }
                            startFromValue = {
                                this.state.startFromValue
                            }
                            handleStartFromChange = {
                                e => this._handleStartFromChange(e)
                            }
                            toggleStartFrom = {
                                () => this._toggleStartFrom()
                            }
                            />
                        ) : (
                            undefined
            ...
            renderOptionsState(props: Object): React$Element < any > {
                return ( <
                    div className = {
                        this.state.view === shareOverlayView.EmbedOptions ? 'overlay-screen active' : 'overlay-screen'
                    } >
                    <
                    div className = {
                        style.title
                ...
                switch (this.state.view) {
                    case shareOverlayView.EmbedOptions:
                        return this.renderOptionsState({
                            title: < Text id = "share.embed_options" / > ,
                            shareUrl: this.getEmbedCode()
                        });
        ...
        // WEBPACK FOOTER //
        // ui/components/share-overlay/share-overlay.js