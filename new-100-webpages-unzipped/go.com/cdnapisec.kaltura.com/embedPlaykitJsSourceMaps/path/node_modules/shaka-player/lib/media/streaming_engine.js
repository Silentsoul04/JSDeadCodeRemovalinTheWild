 ...
 * @return {!Promise}
 */
shaka.media.StreamingEngine.prototype.loadNewTextStream = function(stream) {
    const ContentType = shaka.util.ManifestParserUtils.ContentType;
    ...
    // The Sequence Id is to avoid the risk condition.
...
shaka.media.StreamingEngine.prototype.setupStreams_ = function(streams) {
    ...
    let uniqueStreamIds = streams.map(function(s) {
            return s.id;
        })
...
shaka.media.StreamingEngine.prototype.onUpdate_ = function(mediaState) {
    ...
    // Sanity check.
    goog.asserts.assert(!mediaState.performingUpdate && (mediaState.updateTimer != null),
        logPrefix + ' unexpected call to onUpdate_()');
    if (mediaState.performingUpdate || (mediaState.updateTimer == null)) return;

    goog.asserts.assert(!mediaState.clearingBuffer,
    ...
    if (mediaStates.every(function(ms) {
...
shaka.media.StreamingEngine.prototype.parseEMSG_ = function(
    ...
    if (schemeId == 'urn:mpeg:dash:event:2012') {
        ...
        let event = new shaka.util.FakeEvent('emsg', {
            'detail': emsg
        });
    ...
    this.setupPeriod_(needPeriodIndex).then(function() {
        ...
        for (let type in streamsByType) {
            ...
            if (type == ContentType.TEXT) {
                ...
                this.initStreams_({
                    text: streamsByType[ContentType.TEXT]
