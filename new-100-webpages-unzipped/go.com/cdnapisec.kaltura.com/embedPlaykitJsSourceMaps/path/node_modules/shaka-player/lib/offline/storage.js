...
shaka.offline.Storage.prototype.downloadAndStoreManifest_ = function(
    ...
    return this.storageEngine_.addManifest(manifestDb).then((id) => {
        const OfflineUri = shaka.offline.OfflineUri;
        const OfflineUtils = shaka.offline.OfflineUtils;
        /** @type {string} */
        let uri = OfflineUri.manifestIdToUri(id);
        let downloadPromise = this.downloadManager_.downloadAndStore(manifestDb, id)
            .then(() => {
                return manifestDb.downloadStatus;
            });
        return {
            'manifest': OfflineUtils.createStoredContentFromManifestDB(uri, manifestDb),
 ...
 * @export
 */
shaka.offline.Storage.prototype.storeManifest = function(
    manifestUri, opt_appMetadata, opt_manifestParserFactory) {
    // TODO: Create a way for a download to be canceled while being downloaded.
    if (this.storeInProgress_) {
        return Promise.reject(new shaka.util.Error(
    ...
    return this.initIfNeeded_().then(function() {
        ...
        return this.createAndStoreManifest_(
            manifestUri, data.manifest, appMetadata, data.drmEngine);
    }.bind(this)).then(function(content) {
        this.checkDestroyed_();
        return this.cleanup_().then(function() {
 ...
 * @private
 */
shaka.offline.Storage.prototype.createAndStoreManifest_ = function(
    manifestUri, manifest, appMetadata, drmEngine) {
    const OfflineUtils = shaka.offline.OfflineUtils;
    ...
    return this.storageEngine_.addManifest(manifestDb).then((id) => {
        const videoStreamDb = OfflineUtils.getVideoStreamDb(manifestDb);
        const audioStreamDb = OfflineUtils.getAudioStreamDb(manifestDb);
        const variant = StreamUtils.getVariantByStreamIds(
            audioStreamDb.id, videoStreamDb.id, manifest.periods[0].variants);

        // removing all the regular segments from the segments_ arrays
        this.downloadManager_.resetSegments();

        // adding reference segment to the segments_ arrays
        this.addInitReferenceSegment_(variant.audio, audioStreamDb);
        this.addInitReferenceSegment_(variant.video, videoStreamDb);

        // download only the reference segments to complete the manifest
        return this.downloadManager_.downloadAndStore(manifestDb, id)
            .then(() => {
                const OfflineUri = shaka.offline.OfflineUri;
                const OfflineUtils = shaka.offline.OfflineUtils;
                /** @type {string} */
                let uri = OfflineUri.manifestIdToUri(id);
                let manifest = OfflineUtils.
                createStoredContentFromManifestDB(uri, manifestDb);
                manifest['expectedSize'] = this.expectedSize_;
                return manifest;
            });
    });
};
...
shaka.offline.Storage.prototype.download = function(offlineUri) {
    return this.download_(offlineUri).then(function(manifestDb) {
        shaka.log.info('finished download flow');
 ...
 * @private
 */
shaka.offline.Storage.prototype.download_ = function(offlineUri) {
    const OfflineUtils = shaka.offline.OfflineUtils;
    ...
    return this.initIfNeeded_().then(function() {
        ...
        return this.storageEngine_.getManifest(manifestId);
            ...
            function(manifestDb) {
                ...
                this.mapAllSegments_(manifestDb.periods);
                ...
                return this.downloadManager_.downloadAndStore(manifestDb, manifestId)
                    .then(function() {
...
/**
 * mapping all the periods segments : in a hash-array-index manner
 * @param {!Array.<shakaExtern.PeriodDB>} storedPeriods
 * @private
 */
shaka.offline.Storage.prototype.mapAllSegments_ = function(storedPeriods) {
    storedPeriods[0].streams.forEach((stream) => {
        this.segmentsMaps_[stream.id] =
            shaka.offline.Storage.mapSegments_(stream.segments);
    });
};
...
shaka.offline.Storage.prototype.loadInternal = function(
    ...
    return shaka.media.ManifestParser
        ...
        .catch(function(error) {
            if (manifestParser) {
                return manifestParser.stop().then(function() {
                    throw error;
                });
            } else {
                throw error;
...
shaka.offline.Storage.prototype.defaultTrackSelect_ = function(tracks) {
    ...
    if (tracksByHeight.length) {
        ...
        tracksByHeight.sort(function(a, b) {
...
shaka.offline.Storage.prototype.createStream_ = function(
    ...
    // a map that stores a hash that point to the index of the segment
    // in the array. The hash is consisted of "startTime-endTime" of
    // a segment. The map is populated inside the "forEachSegment_"
    let segementIndexesMap = this.segmentsMaps_[stream.id] || {};
    shaka.offline.Storage.forEachSegment_.bind(this)(stream, startTime,
        function(segment) {
            ...
            const hashString = shaka.offline.Storage.getSegmentHash_(segment);
            // if its already exists in the hashmap, don't replace it. the new
            // segment location is for the not downloaded yet and it will
            // ruin the whole streamDB object.
            if (!segementIndexesMap[hashString]) {
                segementIndexesMap[hashString] = streamDb.segments.length - 1;
            }
            this.expectedSize_ += endByte == null ? bandwidthSize :
                (endByte - startByte + 1);
            this.downloadManager_.addSegment(
                ...
                function(id) {
                    ...
                    if (Object.keys(segementIndexesMap) == 0) {
                        segementIndexesMap =
                            shaka.offline.Storage.mapSegments_(streamDb.segments);
                    }
                    const hashString = shaka.offline.Storage.getSegmentHash_(segmentDb);
                    const index = segementIndexesMap[hashString];
                    streamDb.segments[index] = segmentDb;
                });
        }.bind(this), this.manifest_.storedPeriods);


    this.addInitReferenceSegment_(stream, streamDb);
...
/**
 * create and add initSegmentReference
 *  @param {?shakaExtern.Stream} stream
 * @param {shakaExtern.StreamDB} streamDb
 * @private
 */
shaka.offline.Storage.prototype.addInitReferenceSegment_ = function(
    stream, streamDb) {
    let initSegment = stream.initSegmentReference;
    if (initSegment) {
        const noBandwidth = 0;
...
/**
 * creates a hash per segment
 * @param {shakaExtern.SegmentDB} segment
 * @returns {string}
 * @private
 */
shaka.offline.Storage.getSegmentHash_ = function(segment) {
    return `${segment.startTime}-${segment.endTime}`;
};

/**
 * creates a map per segment
 * @param {!Array<shakaExtern.SegmentDB>} segments
 * @returns {!Object<string, number>}
 * @private
 */
shaka.offline.Storage.mapSegments_ =
    function(segments) {
        let map = {};
        segments.forEach((segment, index) => {
            map[shaka.offline.Storage.getSegmentHash_(segment)] = index;
        });
        return map;
    };
