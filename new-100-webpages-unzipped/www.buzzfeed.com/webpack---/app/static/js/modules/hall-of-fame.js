...
import {
    fetch
} from '@buzzfeed/buzzblocks/js/services/recsys-api';
...
Application.addModule('hall-of-fame', function(ctx) {
    ...
    function render(quizzes) {
        element.innerHTML = template.render({
            image_item: convertItemData(quizzes.shift(), {
                number: 0
            }),
            text_items: quizzes.map((quiz, i) => convertItemData(quiz, {
                number: i + 1
            })),
        ...
        Application.broadcast('hall-of-fame-active', {
            filter: 'quizzes'
        });
    }
