...
const getTrackListBySource = (albumData, sourceToPlay) => {
    ...
    if (sourceToPlay === 'archive') {
        if (playSamples) {
            const sampleMP3s = tracks.map((track, index) => {
                ...
                return Object.assign({}, sampleMP3, {
                    trackNumber,
                    ...trackDetails
                });
            });
        ...
        return tracks.map((track, index) => {
            ...
            const trackNumber = index + 1;
            return Object.assign({}, trackToUse, {
                trackNumber,
                formattedLength
            });
        });
    ...
    if (externalSource) {
        ...
        const tracksWithExternalSource = tracks.reduce((allTracks = [albumPlaceholder], track, index) => {
            ...
            const formattedLength = !trackLengthFormatted ? formatTime(length) : length;
            const trackNumber = index + 1;
            const formattedTrack = Object.assign({}, track, {
                formattedLength,
                trackNumber,
                ...trackDetails
            });
            if (track.hasOwnProperty(sourceToPlay)) {
                allTracks.push(formattedTrack);
            }
            return allTracks;
        }, []);
